...
- The cold-start figure the ADR needs, measured: source-at-runtime means every invocation transpiles the TS entry graph (644 `.ts` files, ~50.6k non-test LOC) through `tsx/esm/api` before any work starts — `noldor --help` 81ms, `noldor triage validate` 371ms on a warm macOS checkout. That tax lands on every lefthook job (trailer injection, commit-msg validation, pre-push summary-body gate) on every commit, so it is operator-felt latency and not only install and bandwidth waste. It also argues for weighting option (2) above option (1): shipping only runtime source keeps the transpile, while making `dist` canonical removes it. Adds one implementation shape the three options do not cover — point `bin/noldor.mjs` at `dist/cli/index.js` and keep a `tsx` fallback for source checkouts, so `pnpm noldor` from this repo and consumer development keep working without publishing `src`; bundling to a single file (esbuild) is the further step that also collapses module resolution. Two assumptions move with the entrypoint: `src/core/prerequisites.ts:49` currently asserts the opposite invariant ("bin/noldor.mjs + tsx runtime execute all CLI surfaces"), and `src/core/noldor-cli.ts` documents the tsx boot in its docstring. (measured 2026-08-17 assessing a Go-rewrite question)
