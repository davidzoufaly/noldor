...
- `scripts/migration/` — five one-shot extraction scripts (`classify.ts`, `classify-feature-track.ts`, `cross-tree-link-audit.ts`, `partition-blocks.ts`, `stage-framework-docs.ts`) plus four tests. Referenced only by archived specs/plans (`docs/superpowers/{specs,plans}/archive/2026-05-28-framework-doc-extraction-*`); no `package.json` script, no pipeline, no live doc points at them. They ran once for PR #57 (Phase B doc migration).
