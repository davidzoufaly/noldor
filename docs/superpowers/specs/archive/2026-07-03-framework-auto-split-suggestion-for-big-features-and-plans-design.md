...
- `noldor-plan` skill (`.claude/skills/noldor-plan/SKILL.md`): new step between self-review (5) and save-report (6) — after writing the file, run `split-check --plan <path>`; on exit 2, report the P1 signal and restructure into `docs/superpowers/plans/YYYY-MM-DD-<slug>-part<N>.md` parts (each part independently shippable, per the skill's existing "one plan per subsystem" rule in step 1) before reporting done.
