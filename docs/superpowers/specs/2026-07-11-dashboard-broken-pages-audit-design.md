...
Reads `.noldor/drain-state.json` (schema: `src/autonomous/drain-state.ts:DrainState`), `.noldor/drain-park.json` (writer: `src/autonomous/escalations.ts`, `PARK_REL`), and reuses `loadWatchLogTail`. `pidAlive` reuses the injected `isPidAlive` pattern from `loadAgentActivity` (`AgentActivityDeps`) so tests stub it. All three reads are individually failure-tolerant (missing/corrupt file → null / empty), matching the existing `try/catch → {}` idiom at `data.ts:2120-2127`.
