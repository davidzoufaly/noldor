...
- **Collect:** per task, `parseResearchStdout`; write `<id>.findings.md` (findings + an HTML comment header carrying only enum/kebab-safe fields — id/status; free text — the `question`, `spawnStatus` with raw error messages — lives in `manifest.json` and INDEX, never inside the comment, so no escaping/mangling is needed); timeout/non-zero exit recorded in `spawnStatus`, meta forced to fallback when stdout empty.
