...
5. Below `minTokens`/`minLines` → no group. Test-file exclusion honored; `--include-tests` includes. Repetitive single run: NO group has overlapping instances (guard holds pre- and post-extension); a long tandem repeat MAY surface as one adjacent half-split pair — assert instances disjoint + group count ≤ 1, not zero.
