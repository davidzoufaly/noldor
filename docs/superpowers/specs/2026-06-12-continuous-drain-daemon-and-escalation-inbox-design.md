...
- skip with reason `retries-exhausted` or `pr-open-unmerged` (K=1 paths), or either coordinator outcome string — `merge-conflict — PR left open for human resolution` / `merge-timeout — PR left open for human resolution` (both non-`merged` members of `MergeOutcome`, `drain-io.ts:16`, written at `drain-loop.ts:268`) → append escalation (reason `retries-exhausted` / `pr-open-unmerged` / `merge-conflict` / `merge-timeout`) + park the slug. The open-PR cases share one suggested action: "PR open but unmerged after gate exit — check auto-merge/CI, merge or close the PR, then unpark". Without these, an unmerged PR is silently re-skipped by `openPrExistsFor` every later cycle — the invisible-failure class this feature exists to kill.
- Dedup: a slug already present in `drain-park.json` is never re-escalated by a later cycle (one inbox row per open incident; the JSONL keeps the first, authoritative evidence).
