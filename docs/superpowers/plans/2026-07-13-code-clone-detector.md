...
- [ ] **Step 1: implement** — in `main()` after the Summary block: build the clone corpus with `walkCodeFiles` over `resolveScanRoots` (deliberate: one extra directory walk over ~350 files, chosen so the clone corpus has EXACTLY ONE policy source — filtering the existing `allRepoPaths` (from `walkRepo`, whose directory-exclusion set differs) by extension regexes would silently mix two directory policies; policy consistency beats saving one sub-second walk), read the files, `detectClones` with config/default opts, push `## Code clones` + `- N clone groups, X.Y% duplicated tokens across M files` + top-5 groups as `- path:start-end and path:start-end (N tokens)` bullets (no `_`/`*` in generated strings — oxfmt gotcha).
