...
The milestone file must be final before the verdict, because the record binds its blob: add the `## Architecture target` section first, then take the verdict. The skill still never commits. The operator's micro-chore commit carries the milestone file, the target and its record, all of which Part 8 put on the lane.

- [ ] **Step 1: Add the target step to `draft` and `edit`.**

  In `.claude/skills/noldor-milestone/SKILL.md`, in `### /noldor-milestone draft [<slug>]`, directly after step `3. Run tsx src/milestones/cli.ts draft <slug> [<description>] from the repo root.`, insert:

  ```markdown
  3.5. **Target architecture (optional, when `docs/design/architecture/baseline.pen` exists).** Ask whether to sketch the milestone's target architecture. On yes:
     ...
     3. Add an `## Architecture target` section to `docs/milestones/<slug>.md` linking `../design/architecture/milestones/<slug>.pen` — **before** the verdict, because the record binds the milestone file's text.
  ...
  ```markdown
  4.5. When the milestone has a target (`docs/design/architecture/milestones/<slug>.pen`): a revised target is an edit to the `.pen` plus a fresh verdict (step 3.5's step 4 — it overwrites the record); an edit to the milestone file alone shows as drift in `pnpm noldor design verdict --pen docs/design/architecture/milestones/<slug>.pen --check` — re-take the verdict, or `--reconfirm` when the target still stands.
  ```

- [ ] **Step 2: Print progress at `activate`.**

  In `### /noldor-milestone activate <slug>`, directly after step `2. On success, surface: …`, insert:

  ```markdown
  2.5. When the milestone just flipped to `shipped` has a target (`docs/design/architecture/milestones/<that slug>.pen`), run `pnpm noldor design arch-progress --milestone <that slug>` and surface its report. Advisory — it never blocks the activation; `to-build` rows are what the shipped milestone left undone.
  ```
