...
**Architecture:** New `src/core/agent-runner/` owns runner identity (binary names, argv builders, capabilities, role resolution, spawn). All call sites consume `spawnAgent()` or the argv builders; nothing outside the registry (plus `src/cr/deep-review-spawn.ts`'s single-sourced binary reference) names an agent CLI. Spec: `docs/design/specs/2026-06-11-make-noldor-agent-agnostic-design.md`.
