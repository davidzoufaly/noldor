...
**Architecture:** Mirror of the subagent lane — new `RunLane` in `src/cr/lanes/verify.ts` registered in orchestrate's `LANES` dispatch; agent spawned headless via `spawnAgent(role: 'verifier')`; deterministic smoke module in `src/verify/` with a CLI manifest entry. Boot knowledge in `consumer.verifyCommands`; policy knob `autonomous.verifyMode`. Spec: `docs/design/specs/2026-06-12-acceptance-verify-lane-design.md`.
