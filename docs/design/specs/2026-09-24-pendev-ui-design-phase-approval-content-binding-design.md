...
A `.pen` that is not JSON, has no `children` array, or has a page with no string `name` is refused with exit 2 (fail closed). Surface check (Open question 5): each `--surface` must own at least one `FINAL:<surface>:` page on disk, and each `FINAL:` page must name a passed surface. That turns `surfaces` from descriptive metadata into a checked claim.

### D2 — Spec binding: what else the record names

`--approve` gains a required `--spec <path>`. It must resolve inside the specs root (`loadDocRoots(repo).specs`, its `archive/` included, for the same reason `--pen` accepts an archived design: gate Step 4 archives both before the code-stage lane runs), follow the `<date>-<key>-design.md` scheme, and carry the same dialogue key as the `.pen` (`specSlugFromFilename` against `penSlugFromFilename`). The record stores `spec: { name, blob }`: the spec's basename, and its blob id computed with `git hash-object -w` so the approval-time text stays in the object store for a later diff.
...
- **Spec stage (first catch).** `design verdict --check --pen <p>` is read-only. It reads the record, finds the spec it names (D2), and compares that file's blob with `spec.blob`. Exit 0 when they match, or when the record is `waived` (nothing was ratified, so nothing can drift). Exit 1 when they differ, printing `git diff <spec.blob> <current>`, or a note saying the approval-time text is not in this clone. Exit 1 also for a record with no `spec` (remedy: take the verdict again). Exit 2 when there is no usable record or the named spec is gone. Gate Step 2.5 runs it before the continue-dialog on a UI-bearing session (`uiVerdict: required`, no `uiWaiver`). On 1 the operator reads the diff and either confirms the design still depicts the spec (`--reconfirm`), or sends the design back to be revised and approved again.
...
- **Code stage (backstop).** `resolveUiReviewTarget` runs its existing `design-approval-stale` check, then reads the spec the record names from the review-head tree — the same tree it already reads the `.pen` and the record from, so no comparison spans two revisions. When that blob differs from `spec.blob`, it terminates `cannot-review` with a new reason, `design-approval-spec-stale`: red under `blocking`, green with a note under `advisory`. A named spec that is in neither place at the review head terminates the same way, naming the missing file. A record with no `spec` is reviewed as today, plus a note naming the remedy.
...
4. A `.pen` that is not JSON, lacks a `children` array, or holds a page with no string `name` makes `--approve` exit 2 with nothing written.
...
6. `--approve` without `--spec`, with a spec outside the specs root, or with a spec whose dialogue key differs from the `.pen`'s, exits 2. On success `spec.name` is the spec's basename, `spec.blob` is its blob id, and `git cat-file -e <spec.blob>` succeeds afterwards.
7. `design verdict --check` finds the named spec whether it is live or archived. It exits 0 when that file's blob equals `spec.blob` or the record is `waived`, and exits 1 with the spec diff printed when the blob differs. A record without `spec` makes it exit 1 with the remedy named.
...
10. A record written before this change (no `pages`, no `spec`) still parses, and still satisfies `checks shared-files` for its `.pen`.
11. Both copies of the `noldor-spec` skill take the verdict after self-review and pass `--editor-page` for every top-level page and `--spec`; both copies of the gate skill run `design verdict --check` on UI-bearing sessions at Step 2.5 and route a drift to confirm-or-revise.
...
6. *What happens to records written before this change?* -> They parse; `--check` exits 1 and the lane adds a note, each naming the remedy: take the verdict again (D5).
