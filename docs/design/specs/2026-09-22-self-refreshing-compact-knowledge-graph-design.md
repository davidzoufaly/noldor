...
8. The job checks out a merge commit by explicit sha, and `graphifyy` is installed at a pinned version — so a run's output is a function of that sha, not of when the run happened.
...
18. Node ordering within a community is identical under `LANG=cs_CZ.UTF-8` and `LANG=en_US.UTF-8`.
