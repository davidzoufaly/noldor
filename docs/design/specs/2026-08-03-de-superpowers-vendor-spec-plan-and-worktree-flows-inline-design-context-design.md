...
- `design log --slug s --scope $'## Open'` leaves the ledger with exactly four `## ` headings plus `## Entry` (no forged section); `design context` renders `## Open` as the Scope text and reports zero open threads.
