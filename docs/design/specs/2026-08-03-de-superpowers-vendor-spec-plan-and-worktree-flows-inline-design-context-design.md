...
Single purpose: hold the running design state for one dialogue, on disk, as append-only markdown with fixed H2 sections.
...
- Four sections, written only by code (Unit 2): `## Scope`, `## Decided`, `## Open`, `## Existing support`.
...
```
pnpm noldor design log --slug <slug> [--scope <text>] [--decide <text>]... [--open <text>]... [--resolve <id>]... [--support <text>]...
```

- Each flag is repeatable; a single invocation can resolve `O2` and record the `D5` it produced.
...
- Unknown `--resolve` ID → exit 1 with the known IDs listed. A typo must not silently drop a resolution.
...
```
pnpm noldor design context --slug <slug> [--kind spec|plan]
```
...
**Scope resolution** — `loadScope()` in `src/design/ledger.ts`, first hit wins:

1. the ledger's `## Scope` section (explicit, written by `--scope`);
2. the `docs/roadmap.md` block for the slug, via `parseRoadmap()` from [`src/utils/parse-blocks.ts`](../../../src/utils/parse-blocks.ts) (the same parser [`src/core/next-priority.ts:10`](../../../src/core/next-priority.ts) uses) — matching on `BacklogEntry.slug`, reading `.description`. Present on attach paths until retirement;
3. `## Summary` of `docs/features/<slug>.md` when the entry is already promoted;
...
- `noldor-spec/SKILL.md` step 3 gains: seed `--scope` and `--existing-support` at dialogue start; run `design context` and paste its stdout verbatim immediately above **every** question; after the operator answers, `design log --decide/--resolve` before the next question.
...
- Seed at dialogue start (agent does this before question 1):
  `pnpm noldor design log --slug <slug> --support "src/foo.ts:12 — already does X"`
