...
- A value containing a backtick run — `design log --slug s --decide '``` end'` — cannot close the fence the skill wraps the block in, because every rendered value line carries the `- ` prefix and a fence closer must start its line.
