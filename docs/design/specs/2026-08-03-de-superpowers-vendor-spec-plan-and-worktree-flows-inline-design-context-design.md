...
- **Free text is normalized on append**, so no value can forge a section boundary, an ID, or a resolution. Exactly two rules, both **non-reintroducing** — the output of each rule cannot re-create the pattern that rule removes, which is the property the guarantee rests on (they also happen to be idempotent, which is what the tests assert; the two are not the same property, and a future rule must satisfy the former):
  ...
  **The same protection must hold at render time**, since Unit 4 pastes the renderer's stdout into chat: the renderer never strips the `- ` prefix from a value line (a stripped `## Open` would land at column 0 of the pasted block and forge a heading *there*, defeating the storage guarantee one layer up), and the skill pastes the block inside a fenced code block so nothing in it is interpreted as markdown at all. Two layers, both free.
