...
- **Only tracked files.** The code list keeps only files git tracks, so a scratch file or an ignored build output never enters the graph. CI's checkout holds nothing else, so this changes nothing there.
...
5. A file git does not track never appears in the graph, and a parse cache under the repo's `graphify-out/cache/` is never read.
