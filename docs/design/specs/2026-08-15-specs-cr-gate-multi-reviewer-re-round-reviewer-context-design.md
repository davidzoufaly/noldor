...
- **Scoped-review predicate, named once:** new `src/cr/review-scope.ts` exports `isScopedReview({ baseSha, fullReview })` ≡ `Boolean(baseSha) && !fullReview`. It replaces the inline copies at `src/cr/lanes/codex.ts:27` and `src/cr/review-with-codex.ts:63,72`, and is the predicate `runSubagent` uses below — one source instead of a fifth hand-rolled copy.
- **Two shas, two jobs:** `runSubagent` computes `promptBaseSha = isScopedReview(input) ? input.baseSha : input.artifactSha` for the prompt's range line — equal base and head shas select `buildPrompt`'s existing "if equal, review the whole artifact" branch. The rules-resolution base stays what it is today: `rulesBaseSha = input.baseSha ?? \`${input.artifactSha}~1\``, fed to `resolveBindingRules` unchanged — binding rules must keep resolving over the real change set on full reviews (`git diff <head> <head>` yields no files, which would silently drop the rules section from every code-kind full review).
...
13. `isScopedReview` is the single source of the scoped-review predicate: the codex lane and `review-with-codex` call it, and no inline `baseSha && !fullReview` copy remains in `src/cr/`.
...
9. *Does the whole-artifact intent of `fullReviewOverride` actually reach the reviewer?* -> Not today — the lane ignores `fullReview` and prompts `HEAD~1..HEAD`. Fixed here via `isScopedReview` + the two-sha split: the prompt range collapses to equal shas while rules resolution keeps its real base. (D9)
