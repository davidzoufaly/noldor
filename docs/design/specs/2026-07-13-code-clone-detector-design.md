...
- Self-overlap within one file counts (duplication inside a single file is a real signal), **but the two instances of a pair must be token-range DISJOINT** — a match whose occurrences overlap (offsets `i` and `i+k`, `k <` window, the repetitive-run case: long enums, import blocks) is discarded before extension, AND the guard is re-checked AFTER extension — in a periodic run, two disjoint seeds (offsets `i`, `i+k`, `k ≥` window) each extend to near-full-run instances that then overlap; a pair whose EXTENDED ranges overlap is likewise discarded. Both checks together are the jscpd-style guard; without them greedy extension inflates `duplicatedTokens` with degenerate self-overlapping groups. Known shared limitation (jscpd has it too): a sufficiently long tandem repeat can still surface as one ADJACENT half-split pair (e.g. tokens [0,74] ⇄ [75,149] of a 150-token periodic run) — that pair is disjoint, real duplication, and reported; the guard only bans OVERLAPPING instances. Final step: **containment dedup** — after all groups are built, a group G2 is dropped only when there is an INJECTIVE mapping from G2's instances onto DISTINCT instances of a larger-token group G1, each G2 instance fully contained in its mapped G1 instance (both of G2's instances inside ONE G1 instance does NOT qualify — that G2 expresses internal repetition G1 doesn't), so a short-period run collapses to at most its single maximal pair instead of a family of nested/staggered sub-pairs. A clone must additionally span ≥ `minLines` (default 5) source lines on each side to filter trivial runs.
- Output:
...
5. Below `minTokens`/`minLines` → no group. Test-file exclusion honored; `--include-tests` includes. Repetitive single run: NO group has overlapping instances (guard holds pre- and post-extension); a long tandem repeat MAY surface as one adjacent half-split pair — assert instances disjoint + group count ≤ 1 after containment dedup, not zero.
