...
Underneath all three is one policy failure, not three bugs: the check is a convention that each new path-building call site has to remember. Repository-authored references feed non-slugs deeper into the readers as well, through two schemas that accept any non-empty string: `src/core/feature-schema.ts:115` (`milestone: z.string().min(1).optional()`) and `src/dashboard/data.ts:770` (`visionFrontmatterSchema`'s `current-milestone`). `milestoneFrontmatterSchema` at `src/milestones/lib.ts:9` is *not* one of them — it carries only `name` / `status` / `description`, and a milestone's slug comes from `basename(absPath, '.md')` at `:44`.
...
A new `src/core/slug-paths.ts` is the choke point. It exports one builder per slug-rooted family — `worktreePath`, `worktreePidsPath`, `featurePath`, `milestonePath` — each taking `(cwd, slug)` and returning a result. Each builder calls `parseSlug` first, then joins, then verifies the joined path is contained within its declared root.

Containment is two existing pieces, not one. `contained()` at `src/design/artifact-locate.ts:80` is a pure comparison — `candidate === root || candidate.startsWith(root + sep)` — and does no realpath resolution at all; the resolution is `resolveExisting()`, already shared code at `src/core/branch-added.ts:197`, which `vet()` applies to the candidate at `:96` and to the root at `:207`. Each builder therefore resolves **both** sides with `resolveExisting` and then compares with the lifted `contained()`. Both sides is not optional: the `branch-added.ts` docstring requires same-form-on-both-sides, otherwise a `/var` versus `/private/var` cwd rejects every legal path.

`resolveExisting` also settles the prospective-path case — a worktree directory or a milestone draft that does not exist yet. It resolves the deepest existing ancestor and re-appends the unresolved tail, so a not-yet-created target normalizes without an ENOENT. Building the path *is* the first statement of each of the three families, so a builder that refuses is validation at the start of the exported function.
...
**The three families.** `upWorktree()`, `downWorktree()`, the two phase CLIs, and the milestone entry points each replace their inline `join` with the matching builder and handle the failure branch before any side effect. The milestone exports are `draftMilestone` (`src/milestones/lib.ts:65`), `loadMilestoneBySlug` (`:58`) and `activateMilestone` (`:154`) — the last is what `src/milestones/cli.ts:42` actually calls; `preflightActivate` (`:90`) is private and is reached only through it. The exported library functions propagate the failure to their callers rather than throwing; `main()` in each CLI writes the reason to stderr and returns a non-zero exit code. Blast radius is small: `upWorktree` and `downWorktree` have no non-test callers outside their own `main()`, and the milestone functions have four, so result types cost little here. `createWorktree()` keeps its throw — it is already safe — but sources the message from `parseSlug` so there is one string.
...
**Repository-authored slugs that already reach a path.** Three cases exist today, and none is hypothetical:

- `src/dashboard/data.ts:804` passes vision's `current-milestone` straight into `loadMilestoneBySlug`.
...
All three adopt `milestonePath`. Unit 4 closes the first and third at the schema; the second is *not* covered by Unit 4, because `next-priority.ts:252` reads vision through a hand-rolled `as { 'current-milestone'?: string }` cast rather than `visionFrontmatterSchema`, so no schema tightening binds there. It is covered by the builder alone — which is the argument for having a builder at all.
...
`src/core/slug.ts` exports `slugSchema = z.string().regex(SLUG_RE, ...)`, co-located with its inferred type per the `ts-colocate-schema-type` rule. Two schemas adopt it: `src/core/feature-schema.ts:115` (`milestone`) and `src/dashboard/data.ts:770` (`visionFrontmatterSchema`'s `current-milestone`). A hand-edited non-slug reference then fails validation instead of reaching a milestone reader — which is precisely the `src/dashboard/data.ts:804` path named in Unit 2. `src/core/next-priority.ts:252` bypasses this by casting rather than parsing, so it depends on the builder instead.
