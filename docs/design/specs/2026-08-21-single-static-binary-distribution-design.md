...
- **Extractor contract:** `extractAssets(packPath: string, dest: string, fsImpl = node:fs)` — `packPath` is the embedded pack's runtime file path (a string; Unit 2 obtains it), `fsImpl` is the defaulted injection seam the tests use. Errors are thrown `Error`s carrying the attempted path (boundary contract; no result-object API).
