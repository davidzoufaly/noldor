...
- **Pack format:** one blob file, `assets.pack` — a JSON index (`{ version, entries: [{ path, offset, size, mode }] }`) followed by concatenated file bytes. No tar dependency — Node/Bun have no built-in tar, and a hand-rolled indexed blob is ~60 lines with zero new deps.
