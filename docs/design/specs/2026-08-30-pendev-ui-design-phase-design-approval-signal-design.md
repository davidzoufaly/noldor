...
**Identity: one record per design artifact, keyed by the `.pen` stem** — the basename without extension, at `.noldor/design-approval/<pen-stem>.json`. The bare dialogue key would not be injective: two sessions on the same parent and enhancement produce two dated `.pen` files sharing one key, and a later verdict would silently overwrite the only tree-visible record of an earlier, already-archived design. The stem is unique per artifact by construction (`resolveArchivePlan` refuses a basename collision into `archive/`), and every consumer already holds it: the CLI is given the `.pen` path, the guard has the staged path, the lane has `owned[0]`.
...
| `checks shared-files` (guard) | the **index** entry for the staged `.pen` | the destination oid in `git diff --cached --raw -z` |
...
`src/design/design-approval.ts`, beside `archive-resolve.ts` in the c58 neighbourhood that already owns the artifact-naming vocabulary (`src/core/design-artifact-names.ts`) and that both consumers already import from. It exports the union schema, a strict bytes-to-record parse, an atomic write, and the path helper — the same four things `ui-capture.ts` exports, in the same order.

**The shared helpers lift to `src/core/` rather than being written twice.** `blobIdOfWorktreeFile` (`src/design/ui-capture.ts:79`) already runs `git hash-object --path <p> -- <p>` with exactly D2's rationale, and `parseReceiptBytes` already fixes a parse policy for a `.noldor` receipt; this design would otherwise copy the first twice more (module, lane) and re-derive the second. Both lift into `src/core/` and are imported by `ui-capture.ts`, the new module, the guard and the lane — also what the clone ratchet expects, since a second implementation would trip it.
...
**Error policy.** Absent and unreadable-or-malformed records are distinct at the boundary and collapse to the same refusal downstream — "no usable record", never a pass, since fail-open would make the feature decorative. The parse returns `null` for unusable bytes rather than throwing, matching `parseReceiptBytes`. A write to a stem whose record exists **overwrites**: re-taking the verdict on a revised design is the normal remedy, and refuse-if-exists would make the stale case unrecoverable. `design verdict` exits non-zero having written nothing on: both or neither mode flag, a flag from the other mode (`--reason` with `--approve`, `--surface` with `--waive`), a `--pen` that is not a feature `.pen` or does not exist, or a git hash failure.
...
**How, without becoming impure.** The staged read moves from `git diff --cached --name-status -z` to `git diff --cached --raw -z`, whose records carry the destination object id; `StagedChange` gains a `blob` field, and `parseNameStatus` becomes `parseRawDiff`, keeping its rename/copy expansion (`R` → `delete(old) + add(new)`, `C` → `add(new)`, so a renamed or copied `.pen` is an add and is caught). `decideViolations` stays a pure function over data: it additionally receives `records: (stem: string) => string | null` — the record bytes the commit's tree will hold — and does its own strict parse and oid comparison. The impure caller supplies the lookup from the staged entry when one exists, else `git cat-file blob HEAD:<record path>`. That `HEAD` fallback is what leaves no state without a legal exit: `git add` on an unchanged tracked file stages nothing, so a record committed before its `.pen` (split commits, a reordering rebase, a re-added pen) could never satisfy a staged-set-only test. The requirement is presence *in the resulting tree*, not *in this commit's diff*.
...
- `(f) On approve only, write it` — today writes only the approval sentence into `## Design`; it gains the `--approve` form, run after that sentence lands. `(g)`'s approve-with-reservations passes `--reservation`. `(e)`'s revise writes nothing, unchanged.
...
- `src/core/blob-id.ts` — new. `blobIdOfWorktreeFile` lifted out of `src/design/ui-capture.ts`, plus the shared bytes-to-record parse policy lifted from `parseReceiptBytes` (D3). `src/design/ui-capture.ts` becomes an importer of both rather than their owner.
...
- `src/checks/check-shared-files.ts` — `parseNameStatus` → `parseRawDiff` over `--raw -z`, a `blob` field on `StagedChange`, the `records` lookup parameter on `decideViolations`, two new `BlockReason` members (`pen-unapproved`, `pen-approval-mismatch`) and their remedy messages.
...
10. `decideViolations` reaches every verdict above from its arguments alone — a staged-change list and a record lookup — with no filesystem, config or git access of its own.
...
- **R7 — `parseNameStatus` becomes `parseRawDiff`, and it is exported.** A real API change — name, signature, output shape, plus a required parameter on `decideViolations` — to a file the pre-commit chain depends on. Both symbols are consumed only by `check-shared-files.ts` and its test today, so the blast radius is one file; a partial migration would break commits repo-wide rather than failing a test.
...
5. *Path-presence test, or validated binding?* -> **Valid and matching** (D4). Presence would admit a malformed record, one staged for deletion, and approve-then-edit-then-commit-both — the case the binding exists to catch. `--raw -z` hands `decideViolations` the object id as data, so it stays pure.
6. *Must the record be staged in the same commit, or is `HEAD` enough?* -> **Either** (D4). `git add` on an unchanged tracked file stages nothing, so a staged-set-only test has no legal exit for split commits, a reordering rebase, or a re-added `.pen`. The requirement is presence in the resulting tree.
7. *Does the guard fire on every staged feature `.pen`, and can an override waive it?* -> **Only on an add, and no** (D4). Add-only gives the two points non-overlapping jobs and mirrors `pen-archive`'s own `change !== 'add'` test, while keeping a `.pen` that predates this feature from being hard-blocked. `NOLDOR_ALLOW_PEN_WRITE` authorises the Step 4 baseline write-back; letting it waive an approval requirement would couple unrelated permissions.
8. *Blocking or advisory?* -> **Advisory at the lane, unconditional at the guard** (D4). `loadLaneMode` already defaults to advisory and the posture calls for it; the guard has a one-command remedy in front of it, and one that only warned would leave the deletion test unmet.
...
10. *Are `surfaces` verified, and is a stale record distinct from an absent one?* -> **No, and yes** (D2, D4). The authoritative surface set lives in `FINAL:` pages inside an encrypted file no check can read, so the list is descriptive metadata — non-empty and deduplicated, nothing more. Absent and stale get separate reason codes at both points because their remedies differ: take a verdict at all, versus re-take it on the design as it now stands.
...
12. *Which of the approval sentence and the record is authoritative if one write fails?* -> **The record, written last** (D5). It is the only artifact a gate reads, and that ordering makes the survivable failure the loud one: a sentence with no record is refused at the next commit, whereas a record with no sentence would be a silent claim of ratification.
