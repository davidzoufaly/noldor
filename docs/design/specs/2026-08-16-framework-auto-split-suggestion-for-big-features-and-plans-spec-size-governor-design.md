...
Every surface that enumerates split-check's modes or rules gains S1/S2 + `--spec`, in the same PR as Unit 1/2 (the plan lands the `docs/noldor/` twins as their own `noldor`-scoped doc commit rather than folding them into the code commit via a `Noldor-Sibling-Scope` trailer — either satisfies `validate-noldor-scope`; per-commit granularity is the plan's call). Nothing forces these lists to gain the new mode (`validate-script-catalog.ts` diffs only `src/` link targets), so omission rots silently — but the doc pairs themselves are byte-identical twins enforced by `checks template-sync` at pre-commit and pre-push, so each edit must land in **both** the template and its `docs/noldor/` copy:
