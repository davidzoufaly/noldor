...
- **Two renderings of the FD close-out.** `fd-close.md` and the page's Resume path both list it. The command-parity test (criterion 6) catches a `pnpm noldor` command that `fd-close.md` gains and the page lacks. It does not see the FD body refresh, which runs as a skill rather than a verb, or a reworded rule.
