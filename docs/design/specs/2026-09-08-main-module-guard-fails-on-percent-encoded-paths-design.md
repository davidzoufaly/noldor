...
What is known without the graph, by direct measurement: `src/core/cli-entry.ts` has 22 importers under `src/` today and this change adds 36 more, taking it to 58 and making it the highest fan-in module in `src/core/`. That is the intended shape — it is what "one choke point" means — but it is a real centralisation and the invariant in Unit 5 is what keeps it honest rather than merely popular. The 36 sites themselves are leaves: each is a CLI tail at the bottom of its own module, imported by the router and by nothing else, which is why a 36-file sweep carries far less coupling risk than the file count suggests.
...
Nothing else in any of the 33 files changes. This is deliberate: a sweep that also tidies tails cannot be reviewed as a mechanical diff, and the review cost of 36 files is dominated by whether every hunk is the same hunk.
...
### Unit 3 — normalise the three remaining comparison sites
...
`src/hooks/noldor-pre-push.ts:183` is the third, and it is **not** broken: it already reads `import.meta.url === pathToFileURL(process.argv[1] ?? '').href`, which is the correct comparison and the reason the entry could name a known-good form at all. It moves behind the helper anyway, because Goal 1 and acceptance criterion 1 say no file outside `src/core/cli-entry.ts` compares `import.meta.url` **to anything** — and the invariant in Unit 5, matching comparisons rather than the broken template, would flag it. Leaving it as a named third exemption was the alternative; sweeping it is better, because one exempt-because-it-happens-to-be-right site is a rule a reader has to remember, while zero is a rule they can check. Its inline comment explaining the percent-encoding hazard moves to `isEntrypoint`'s doc comment in Unit 1, where it now documents the shared choke point instead of one site.

That makes **36** comparison sites in total: 33 on-template (Unit 2), two off-template, and this one already correct. Thirty-five are defective; all 36 move.
...
There is deliberately **no allowlist of sanctioned right-hand operands.** The sanctioned form is `isEntrypoint(import.meta.url)` — a boolean-returning call containing no comparison at all — so it cannot match a comparison scan and needs no exception; `invokedDirectly` never mentions `import.meta.url`. An "unless the other operand is a helper call" arm would be a branch nothing reaches. Non-guard uses (`fileURLToPath(import.meta.url)` for a directory, `new URL(…, import.meta.url)` for an asset) are likewise not comparisons and so never match.

Exempt files, named in the plugin rather than discovered: `src/core/cli-entry.ts`, which holds the one sanctioned comparison, and `__tests__/` plus `*.test.ts`, inheriting the exclusion `src/invariants/slug-path-choke-point.ts` already applies. Nothing else is exempt — notably not `src/cli/index.ts`, whose comment at line 34 currently spells the old comparison out verbatim and is stale the moment this sweep lands; Unit 2 rewrites it to name `isEntrypoint`, which is why no exemption is needed. A prose comment that trips this plugin later is a false positive that fails closed and costs a reword, which is the right way for a blocking check to be wrong.
...
6. All 36 swept sites retain their pre-change tail behaviour: same exit codes, same stdout/stderr, same async handling.
...
9. A file added under `src/` (outside `src/core/cli-entry.ts`, `__tests__/` and `*.test.ts`) that compares `import.meta.url` to anything causes `pnpm noldor checks invariants` to **exit non-zero**, naming that file and line — the `'file://' + process.argv[1]` concatenation form included, not only the template-literal form.
...
13. The clone ratchet does not rise: 36 identical one-line conditions calling a shared helper replace 35 identical inline comparisons plus one correct-but-inline one.

## Risks / trade-offs

**A 36-file diff is reviewed by sampling, not by reading.** The mitigation is that 33 of the hunks are byte-identical and the other three are called out individually as Unit 3, so a reviewer can verify the shape once and then verify sameness mechanically. The counter-risk — sweeping in smaller batches — leaves the class half-alive across releases and makes the invariant unlandable until the last batch.

**Centralising raises `src/core/cli-entry.ts` fan-in from 22 importers to 58.** A change to `isEntrypoint` then reaches every CLI entrypoint at once. Accepted deliberately: that is the same property that makes the defect fixable once instead of 36 times, and the predicate is four tokens of logic with a two-directional test table pinning both failure directions.
...
11. *What happens to `src/hooks/noldor-pre-push.ts:183`, the one site whose comparison is already correct?*
    → **Sweep it too; the count is 36, not 35.** (D11) Goal 1 and criterion 1 forbid comparing `import.meta.url` anywhere outside `src/core/cli-entry.ts`, and Unit 5 matches comparisons rather than the broken template, so leaving it would make criteria 1, 9 and 11 mutually unsatisfiable. Naming it a third exemption was the alternative: rejected because one exempt-because-it-happens-to-be-right site is a rule a reader has to remember, while zero is a rule they can check.
