...
- No write-time validation of heading names *against the artifact* — a tag naming no heading is stored and warned about, not refused. The one exception is `--confirm-section`, which must read the artifact to hash it. Separately, both CLIs do refuse a heading name `normalize` would rewrite; that is a check on the value's *shape*, not on the artifact, and Unit 5 explains why it is not optional.
...
One narrowing, stated here because it is the only heading a dialogue cannot reach: a name that [`normalize`](../../../src/design/ledger.ts#L78) would rewrite — one carrying a line terminator or a `~~` run — is **not** addressable. `listHeadings` still finds it, but neither CLI will take it as a `--section`, `--confirm-section` or `--unconfirm-section` value. See Unit 5 for why, and the Risks section for what it costs.
...
With no override, candidates are the files in the kind's root whose extracted slug equals the dialogue slug. A spec resolves to exactly one file; several matches are `rejected`, because two spec generations for one slug have no defensible winner. An override names one file, but for `kind: 'plan'` it expands to that file's whole generation cohort — every sibling sharing its stem, in part-number order. Honouring it literally would hand back a subset of a split plan, and a subset is what the all-or-nothing approval rule forbids: a heading living in a sibling part would read as absent and `--confirm-section` would digest a partial plan. A spec override stays the single file named. Discovery and override share one cohort-assembly path, so the generation check, the duplicate-part check and `vet` cannot diverge between them — an override with its own sibling walk skipped all three. Otherwise a plan resolves to every part: [`extractPlanSlug`](../../../src/core/fd-load.ts#L347) strips `^plan\d+-` and `-part\d+` precisely so one slug matches every part of a split plan, so `paths` holds them all in part-number order, `listHeadings` unions their headings in that order, and `extractSection` searches them in that order. A part-less file *is* part 1, so it collides with an explicit `-part1`. Two checks guard the cohort: every match must share one generation stem — its name minus any `-part<n>` suffix, since `extractPlanSlug` strips both the date and a `plan<n>-` prefix and so collapses two generations onto one slug even when their part numbers do not overlap — and within that generation the part numbers must be distinct. Either ambiguity is `rejected`, because blending generations lets `extractSection` resolve a heading to prose the operator never approved. No match yields `none`. Deriving the filename from today's date is not an option — a dialogue that crosses midnight would miss its own artifact.
...
A heading carrying `~~` or a line break cannot be addressed at all — not focused, not tagged, not confirmed. The dialogue still works, since `--section` is optional and the heading renders in the checklist, but that section can never be signed off. Rare enough to accept and loud enough to diagnose, and the escape hatch is renaming the heading.

Storing a `--section` tag without validating it against the artifact means a typo is caught a question later rather than at the keystroke; the failure mode is one visible warning, not lost data. The single-authority claim survives only in the narrowed form Unit 5 states — one shared locator, one render-time validation point — because `--confirm-section` has to read the artifact to hash it, so `design log` does resolve an artifact after all.
...
6. *What happens to a heading name the ledger writer would rewrite?* -> Both CLIs refuse it, which narrows the addressable set rather than widening `normalize`. Accepting it would store the approval under a different key than the lookup used, so the marker would never appear and `⚠ … matches no heading` would stick forever with nothing to diagnose from — a silent permanent failure traded for one immediate error message.
7. *How does drafted prose reach chat without breaking the fenced block that wraps it?* -> A four-space indent on every prose line, one more than CommonMark allows for a fence or an ATX heading, so both become inert without altering a character of the text.
