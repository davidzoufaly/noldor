...
2. When step 1 removed the file, each import of `.claude/noldor.md` in `CLAUDE.md` and then `.claude/CLAUDE.md` is rewritten in place: it becomes an import of `AGENTS.md` if neither file imports it yet, and is otherwise dropped — a whole line goes, a mid-sentence one keeps its words as a plain path — so the migration writes at most one import and removes none that was already there. A modified copy that step 1 kept keeps its import too.
