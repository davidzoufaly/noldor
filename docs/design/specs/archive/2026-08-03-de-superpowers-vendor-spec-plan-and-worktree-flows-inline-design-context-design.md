...
- **The writer fails closed on a malformed ledger — for *any* section.** Unlike the renderer (which degrades, Unit 3), `design log` writes nothing and exits 1 naming the section and the file. Two distinct reasons, both fatal: for `## Decided` / `## Open`, minting the next ID requires reading every existing one, and continuing from "the highest ID it can still read" would re-issue one; for `## Entry` / `## Scope` / `## Existing support`, the writer reserializes the whole ledger from parsed state, so a section it could not parse would be **erased** by the next write rather than merely ignored. Refusing beats silent data loss. Rendering works throughout, so the dialogue is never blocked.
- **A repeated heading counts as unparseable.** A second `## Decided` would otherwise shadow every earlier decision silently — leaving the unparsed list empty and letting the next write re-mint `D1`, which is the one route around the ID guarantee that the checks above do not cover.
...
- A ledger carrying a duplicate `## Decided` heading parses with `Decided` marked unparsed, and `design log` refuses it — the earlier decisions are neither hidden nor re-minted over.
- A hand-mangled `## Existing support` line also blocks the write and survives on disk (the non-critical sections are erase-protected, not just ID-protected), while `design context` still renders.
- `design log --slug s --decide "--fd is validated too"` records that text verbatim; `--decide --open x` (a known flag in the value slot) exits 1 with `missing value`.
- An empty FD `## Summary` resolves to `(scope not recorded)`, not a blank Scope line.
...
- **`design log` is read-modify-write with no lock.** Two concurrent invocations for the same slug can lose one another's append — the atomic write prevents a torn file, not a lost update. Accepted: a design dialogue is one operator answering one question at a time, and separate worktrees have separate `.noldor/`. A lock would be real cost against a hypothetical race.
