...
### Unit 5 — a choke-point invariant (DESCOPED — not shipped)

**This unit was built and then removed before delivery.** Three code-review rounds found the text scan wrong in both directions, and the fixes did not converge: keying on an equality operator missed string-method spellings and rejected correct code sharing a line; a proximity window then exempted any guard near a sanctioned call while refusing an innocent asset read; and the per-line replacement refused `isEntrypoint`'s own documented two-argument form and ordinary one-line doc comments. A blocking repo-wide gate whose false positives reject correct code is worse than the gap it closes, so enforcement is carried forward as its own roadmap entry with all of those findings as input — an AST-backed or far narrower rule, rather than a fourth text heuristic. The consequence is stated in the FD: nothing mechanical prevents the class regrowing until that lands. The design below is kept as the record of what was attempted.
...
9. ~~A file added under `src/` … causes `pnpm noldor checks invariants` to **exit non-zero**~~ — **descoped with Unit 5.** Carried to the follow-up entry.
10. ~~The new invariant is present in both `invariants` and `makeInvariants`~~ — **descoped with Unit 5.**
11. `pnpm noldor checks invariants` is green on the swept tree. (Now trivially so: the tree registers the same five plugins it did before. The reworded comment in `src/cli/index.ts` still stands on its own merits — it quoted a guard form no site uses any more.)
12. `pnpm typecheck`, the full test suite, and `pnpm noldor checks push-gates` are green.
13. Both ratchets are re-recorded in the same commit as the change that moved them, with the direction named. This criterion originally predicted no movement; the measured outcome, after Unit 5 was descoped, is:
    - **Clones 25796 → 25762 (−34), 231 groups → 230, diff-scoped gate green.** Sweeping 42 guards to one call removed more duplication than the uniform one-liners added. (With Unit 5 in the tree this had been +236, 135 tokens of it the new plugin against `src/invariants/slug-path-choke-point.ts`, whose shape it copied. Worth recording for the follow-up entry: rebaselining does **not** clear the diff-scoped gate — that check asks whether a clone group overlaps a line the change wrote, and no baseline silences it — so `abstraction-cost`'s "decline the wrapper and rebaseline" has no answer for a newly-added second copy.)
    - **Indirection 919 → 924 (+5).** 42 new import edges to one leaf module. This is the centralisation the Risks section accepted at fan-in 22 → 64; a choke point cannot be added without adding edges, which makes this the one ratchet the change legitimately raises.
