...
**`cr autofix record` stops appending and starts annotating.** It runs after the seam has applied a fix, and it writes `applied`, `deferred` and `diffStat` onto the entry **whose `headSha` matches the head it reviewed** — never onto "the last entry", which a delayed or retried `record` would attach to the wrong round. No match is an error: `record` exits 2 saying which head it looked for, and the fixes it already applied stand on disk. Two appending writers would have needed a deduplication key, and the only key available does not work — `record` hashes the blockers it just *fixed* while orchestrate hashes the *next* round's, so a `(headSha, fingerprint)` pair matches only in the no-progress case. Making orchestrate the sole appender removes the question, and it removes the `origin` field with it.

**The entry.** One normative shape, so two implementations cannot diverge:

| field | required | written by | meaning |
| --- | --- | --- | --- |
| `round` | yes | orchestrate | entry ordinal in the series, an identifier and not the printed counter |
| `headSha` | yes | orchestrate, refreshed by `record` | the head this round reviewed; `record` re-points it to the post-fix tip so `resolveBaseSha` and `record`'s own `diffRange` rung keep the "previous round's head" meaning they have today |
| `fingerprint` | yes | orchestrate | `fingerprintBlockers` over the round's blockers; `''` is not legal, so a green round hashes its empty blocker list |
| `verdict` | no, absent reads `red` | orchestrate | `green` when the round's `aggregate` reports `ok` |
| `closingRound` | no, absent reads `false` | orchestrate only | the closing-round sentinel |
| `applied` / `deferred` / `diffStat` | yes | orchestrate writes `0` / `0` / `''`, `record` overwrites | what the seam applied |
| `diffRange` / `stopped` | no | `record` | free-text audit fields with no control meaning |

`origin` is gone; an entry that still carries it parses and the field is ignored, and annotation neither preserves nor rewrites it.

**The cap counts red rounds.** `redRounds` is the number of entries whose verdict is `red`; orchestrate refuses to dispatch when `redRounds > AUTOFIX_ROUND_CAP`. With the cap at 2 that is three red rounds — the initial pass plus two re-rounds, the budget the gate prose already describes.

Counting red rounds rather than dispatches is what makes the FD's promise true: a round counts only when it arbitrates unresolved blockers. A green dispatch is free, however many there are. That matters more than it first appears, because the `HEAD^{tree}`-bound receipt is stripped by every fix commit, so a code-stage session routinely runs several green, finding-nothing dispatches purely to re-mint. None of them may cost budget.

**The counter is red rounds over the cap, numerator included.** Fixing only the denominator makes things worse rather than better: `decide` computes its round as `priorRounds.length + 1` and `record` prints the entry ordinal, and both count green entries too, so a session with two green dispatches would print `5/3` beside orchestrate's `3/3` — a numerator above its own denominator. Every printed counter therefore reads red rounds over `AUTOFIX_ROUND_CAP + 1`. `AutofixRound.round` stays the entry ordinal, because it identifies an entry rather than measuring the budget.

**The closing round handles the one case greenness cannot.** The receipt shape on record is *red final round → operator fixes → one dispatch that finds nothing*. At the cap that dispatch is refused and the session is wedged, so the refusal carries a discriminator: orchestrate refuses when `redRounds > AUTOFIX_ROUND_CAP` **and** either `HEAD` equals the last recorded round's head, or the series already holds a `closingRound` entry. A changed head means a fix was committed since the cap was hit, and that earns exactly one further dispatch.

The sentinel is its own boolean, not a value in `stopped`. `stopped` is caller-writable free text set from `record --stopped <reason>`, a documented public flag, so a sentinel living there could be forged by any caller into a permanent refusal for the pair. `closingRound` is written by orchestrate alone.

**The sentinel is written after the receipt, not before.** A closing round that goes green marks itself only once `amendSubagentReceipt` has succeeded. Recording it first would let a crash — or a failed amend — leave a session with neither a receipt nor a permitted retry, which is the wedge this whole mechanism exists to avoid.

**Both seam rules identify the current round rather than trusting position.** `runPlan` in [`src/cr/autofix-cli.ts`](../../../src/cr/autofix-cli.ts) recomputes `fingerprintBlockers` over the *same sinks* orchestrate just hashed, so `no-progress` comparing against every entry would match the one orchestrate wrote moments earlier and decline on every round — the seam would never run again. `prior-deferred` has the mirror problem: orchestrate appends `deferred: 0` before the seam decides, so a rule reading that entry sees a placeholder instead of the preceding annotated round's real count. Both rules therefore exclude **the entry whose `headSha` is the head being decided**, and both read every other entry. Position is not a safe proxy: `plan` can run over sinks no orchestrate dispatch produced, and there the last entry belongs to some earlier round whose blockers must still be compared.

**Session scoping is unchanged, but the sentinel is session-scoped.** `readLedger` returns `null` for a different session and `appendRound` then replaces the series, which is correct and stays. Orchestrate keys on the same value the seam does: `sessionKey` moves from `autofix-cli.ts` beside `isSameSeries` and both callers import it rather than re-deriving the expression — the drift `isSameSeries` exists to prevent. Its `?? ''` fallback is **kept** for counting, because the seam's docblock already reasons about it: rounds accumulate across unrelated sessionless runs, which over-counts and therefore caps early, never late.

The `closingRound` sentinel does **not** inherit that fallback. A permanent refusal is not a conservative early cap, and under a shared empty key one sessionless run's closing round would refuse every future sessionless dispatch for the pair forever. A series with no real session key never records the sentinel, so its refusal always lifts on a changed head.

**Concurrency.** Orchestrate appends and `record` annotates, so this is one file with two mutating processes even though only one appends. The cap is also checked before dispatch and written after. Neither race occurs today: parallel drain assigns each child a distinct slug, the ledger is keyed per `(slug, kind)`, and a gate session runs orchestrate and `record` in sequence. Recorded as `noldor:cut one mutating process per (slug, kind) at a time — the gate runs orchestrate and record in sequence and parallel drain gives each child its own slug; revisit if two agents ever review one pair concurrently, at which point both the append and the annotation need a lock rather than a tighter check`.
...
The append fails open in the same direction and for the same reason. A failed write, or a process killed after the round resolved but before the append, loses that round from the count. Both are logged at error level, and neither changes the round's own result or its receipt — a review that ran and produced findings must report them whether or not its bookkeeping landed. So "every resolved dispatch appends an entry" is a statement about the successful path, and a lost append means the cap under-counts. That is the safe error for a first enforcement pass, and it is the reason the counter is never treated as an audit record: it bounds a loop, and Q-0209's arbitration record is what will carry the account.
...
Unit 2 covers the first dispatch appending an entry, which is what makes the counter bootstrap at all; a round's verdict following its aggregate across a blocking lane, an unresolved lane and a clean run; `record` annotating by head match, and erroring when no entry carries that head; a run of green rounds never advancing the cap or any printed numerator; the refusal at the cap with its exit code and printed history; the refusal holding at an unchanged head and lifting exactly once at a changed one, with the sentinel written only after a successful receipt amend and every later dispatch refused; a forged `--stopped closing-round` not refusing anything; both seam rules skipping the entry for the head being decided while still comparing an older last entry, which is the `no-progress` regression and the `prior-deferred` placeholder together; a sessionless series never recording the sentinel; and a pre-change ledger parsing, keeping its series and reading as red.
...
2. A `cr orchestrate` dispatch that resolves appends one ledger entry for its `(slug, kind)` pair, the first included, with a verdict of `green` when the round's aggregate reports `ok` and `red` otherwise. A dispatch that crashes before resolving appends nothing.
3. `cr autofix record` annotates the entry whose `headSha` matches the head it reviewed, and exits non-zero naming that head when no entry matches; an auto-fix cycle and its re-review report two rounds rather than three.
4. Green rounds never count toward the cap, however many run.
5. `cr orchestrate` refuses to dispatch when red rounds exceed `AUTOFIX_ROUND_CAP` and either `HEAD` matches the last recorded round's head or the series holds a closing-round entry; it exits 3 and prints the round history and the `Noldor-Path-Override` remedy.
6. At the cap with a changed `HEAD` and no prior closing round, exactly one further dispatch runs; a green one amends the receipt and is marked the closing round only after that amend succeeds, and every dispatch after it is refused.
7. Every printed round counter, in orchestrate and in the seam, reads red rounds over the same denominator, and no counter can exceed it.
8. The closing-round sentinel cannot be set through `cr autofix record --stopped`.
9. Both `no-progress` and `prior-deferred` exclude the entry for the head being decided and read every other entry, including when the last entry belongs to an earlier round.
10. A ledger written before this change parses, keeps its series, and its entries read as red.
11. A failed append is logged, leaves the round's own result and the receipt unchanged, and under-counts rather than blocking. A corrupt, unreadable or absent ledger leaves dispatch, aggregate and receipt behaviour exactly as today.
12. A series with no session key never records a closing-round sentinel, and the gate skill, `cr-pipeline.md` and `script-catalog.md` no longer assert the replaced behaviour, with their `templates/` twins matching.
...
**A single appender moves work into orchestrate.** `cr autofix record` becomes an annotator, so a ledger entry now depends on orchestrate having written one first, and a `record` run outside a dispatch fails instead of recording. That is the safe direction — a silent no-op would lose the applied/deferred counts two seam rules depend on — but it is a new way for the seam to error, and the message has to name the head it looked for or the failure is undiagnosable.
