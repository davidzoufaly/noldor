...
After an interruption, which side of Seed a waiver fell on is recovered from whether any `docs/design/ui/*-<dialogue-key>.pen` exists — matched on the dialogue key, never the date, since a session resumed the next day would probe a path Seed never wrote. Seed is what creates the file, so no extra marker field is required. Every waiver after Seed behaves alike, so a bridge that dies mid-iteration is not a third case: the seeded `.pen` is kept either way. `links.design` is normally set by **Record**, which runs before the verdict and needs no MCP — and when the bridge died before Record ran, the waiver step sets it. It stays set: it names a committed-but-unapproved design, and clearing it would make the `rewriteArtifactLinks` repoint pass a no-op, so the FD would end the session with no link to the archived design. Archival itself does not depend on it — `resolveArchivePlan` finds the `.pen` by filename dialogue key under the `branchAdded` gate and never reads `links.design` (D5). The FD `design:` field is a different thing entirely — an operator override, never a verdict record, and never written by this step.
...
2. A `FINAL:` set that is not exactly one page per affected surface — zero, several, or a page naming a surface outside `affectedSurfaces` — routes back to Iterate without consuming a revise round.
