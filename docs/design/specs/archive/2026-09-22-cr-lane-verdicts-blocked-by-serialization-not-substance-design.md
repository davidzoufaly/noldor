...
- Agent lanes hand back a structured answer through a channel that never mixes it with chatter: a JSON answer file. The agent writes it itself, or, for a runner that enforces a schema (codex), the runner's CLI writes it from the schema-validated final message. This replaces the two grammars in use today, fenced JSON and markdown buckets, both dug out of stdout.
...
No agent lane (reviewer, verifier, ui-reviewer, render-export) parses stdout any more. Every dispatch gets its own answer path, built by a new `laneAnswerPath(root, slug, kind, lane, dispatchId)` beside `laneSinkPath` in `src/cr/filename.ts:14`. That puts it through the same `slugPath` choke point as every other per-slug CR path, and yields `.noldor/cr/answers/<slug>-<kind>-<lane>-<dispatchId>.json` with a fresh random `dispatchId` per dispatch. The path is gitignored with the rest of `.noldor/cr/`. It sits in a subdirectory because `aggregate`'s flat `readdir` (`src/cr/aggregate.ts:77`) reads every top-level `.json` there as a sink. Before spawning, the seam creates `.noldor/cr/answers/` (a recursive `mkdir`). If that fails, the lane records it as a dispatch failure on its existing path. The prompt tells the child to write its answer to that absolute path as one JSON object and nothing else. After the child exits, the lane reads only that path and validates it with the lane's zod schema: `verifyVerdictSchema`, `uiReviewReportSchema`, `renderExportReportSchema`, or the reviewer schema from Unit 2. Fences, quoted code and prose no longer matter, because nothing has to find the answer inside the chatter. A fresh id means the file cannot exist before the child writes it, so a dispatch never reads an earlier round's answer, and concurrent dispatches never read or delete each other's. Once read, valid or not, the file is renamed in the same directory to `<slug>-<kind>-<lane>.json`, which keeps the latest raw answer per lane for debugging. The rename is atomic, and last writer wins only for that debug copy.

Who writes the file depends on the runner, and the capability table says which. `RunnerCapabilities` (`src/core/agent-runner/types.ts:22`) gains `answerFile: 'agent-writes' | 'cli-writes'`. claude, opencode and stub are `agent-writes`: the child writes the file with its own tools, which all three may do today (`bypassPermissions`, `--auto`, and an in-repo node process). codex is `cli-writes`. Its sandbox stays `read-only` (`src/core/agent-runner/runners/codex.ts:15`), and the codex CLI itself writes the child's final message to the path through `--output-last-message <path>` (codex-cli 0.156.0), which `buildCodexArgv` gains as a `lastMessagePath` option. The lane validates that message with the same zod schema as any other answer. `--output-schema` is deliberately not used. Codex's strict structured-output mode rejects a schema unless every property is required, so each lane schema would need a separately maintained strict twin. An answer that is not the JSON object is simply an invalid answer, and it takes the repair round. Because the field is a required member of an exhaustive `Record<RunnerName, RunnerCapabilities>`, a runner added later cannot compile without declaring its channel. So there is no runner with no way to answer, and no runner's sandbox is widened to let it write.

The prompt and the spawn options both depend on that channel, so the seam resolves the runner exactly once per dispatch, before building either. It calls `resolveRunner(role, loadAgentsConfig(cwd))` (`src/core/agent-runner/registry.ts:41`) and pins the whole result, runner and model. That needs `opts.runner` plus a new `opts.model` that `spawnAgent` honors alongside the pin. Today a pin keeps only the runner (`registry.ts:117`) and would silently drop a role's configured model. With both pinned, `spawnAgent` cannot resolve differently, and a model set under `agents.roles` survives. It then builds the matching answer instruction. For `agent-writes` that is "write your answer JSON to this path". For `cli-writes` it is "your final message is the JSON object". It also passes `lastMessagePath` only for `cli-writes`. The registry gains the matching capability check beside the `schemaPath` one (`registry.ts:121`), and it refuses `lastMessagePath` for a runner whose child writes the file itself. Either way, every lane reads its answer from the answer file, and none parses stdout.

A missing file, invalid JSON and a schema mismatch are one class ("no answer"), handled by Unit 4. Stdout is kept only as evidence in `notes` when that happens. `parseLastJsonFence` and `parseFencedJson` (`src/cr/extract-json.ts:38-66`) are deleted, but `extractJsonObject` stays because codex uses it. `fencedJsonInstruction` (`src/cr/lanes/prompt-parts.ts:12`) becomes `answerFileInstruction(path, shape)`.
...
With no issues, `findings` is `[]`: there is no bucket to leave empty and no bullet to misread. `runSubagent` reads and validates the answer file. Each finding gets an effective blocking value (Unit 3). An effectively blocking finding becomes a blocker (critical→high, important→med). Every other finding becomes a suggestion (critical or important→med, minor→low). `class` becomes a field instead of the `[mechanical]` prefix. A blocking finding without one reads as `design`, just as an untagged bullet does today, so `cr autofix` never treats it as auto-fixable. `locations` are still derived from the message through `extractLocations`. `parseSubagentMarkdown` is deleted. `splitClassTag` stays only to lift a leftover `[mechanical]` or `[design]` message prefix into `class` when the field is absent, so a model still writing the old tag does not leave it in the message. JSON escaping in long, code-heavy messages is the one new failure surface this adds, and the repair round (Unit 4) backs it up (see Risks).

### Unit 3 — Every finding says whether it blocks

Whether a finding blocks stops being a side effect of which bucket it landed in. Each reviewer finding carries `blocking`. The lane turns that into an effective blocking value, which is true only when three things hold: the finding says `blocking: true`, its severity is not `minor`, and its message (trimmed) does not start with `maybe:` or `unverified:`, compared case-insensitively. So the "never blocks" classes below are enforced in code, not just requested in the prompt. The sink mapping (Unit 2), the blocker count, the verdict and `ok` all read that one value. The verdict is `approve` when no finding is effectively blocking, otherwise `blockers found (N)`. The lane writes the verdict as the sink `summary`, and the model's own one-line assessment moves to `notes`. Today `ok` comes from the blockers while the summary is the model's text (`subagent.ts:287`). After this change both come from one source and cannot disagree.
...
The verifier keeps its own verdict vocabulary. Its schema gains a refinement: `pass` has no mismatches and `fail` has at least one. An answer that breaks it counts as invalid.

As defence in depth, the shared answer reader drops placeholder entries before schema validation. Normalization works only at the ends of the text. It repeatedly strips whitespace and the characters `- * _ . , : ; ! ( ) [ ] { } < >` from the start and the end, never from inside. It then collapses any inner run of whitespace to one space and lowercases the result. A placeholder is an entry whose normalized text is empty or exactly one of `none`, `n/a`, `na`, `no issues`, `no findings` or `nothing`. Matching is exact, so `none of the tests assert the exit code` is kept. Each lane names the field it filters: the reviewer's `findings[].message`, the verifier's `mismatches[]` and the ui-reviewer's `findings[].message`. Render-export reports no findings. Because the filter runs first, a `pass` padded with `["(none)"]` validates as the clean pass it is. A `fail` whose only mismatches are placeholders fails its refinement, so it is treated as an ambiguous answer and goes to the repair round (Unit 4) to name the mismatch.

### Unit 4 — One repair round, every agent lane

Today only the verifier has a repair round (`src/cr/lanes/verify.ts`, step 4): one cheap re-request that hands the child its own prose and asks it only to transcribe that into the schema. The shared dispatch seam (`createDispatcherSeam`, `src/cr/lane-spawn.ts:57`) gains that behaviour, so the reviewer, ui-reviewer and render-export lanes get it too. The reviewer currently spawns through its own `dispatcher` in `subagent-dispatch.ts`, so it moves onto the seam and keeps `setDispatcher` as its test injection point. When the answer is missing or invalid after a clean exit, the seam dispatches once more under a new `dispatchId`. It hands over the child's stdout, the rejected file content and the validation error, and asks only for the answer to be written. Each lane supplies its own transcription prompt, with the rule that a hedged report never becomes pass or approve. The repair is recorded in `notes`. An answer still missing or invalid after the repair takes the lane's existing "no trustworthy verdict" path, unchanged. A timed-out or crashed dispatch gets no repair, as today.
...
- A reviewer answer whose only findings are placeholders yields zero blockers (Q-0246 replay). `(none)`, `- None.`, `**N/A**`, `[]` and an empty message are all dropped, while `none of the tests assert the exit code` is kept.
- A reviewer finding lands in the blockers exactly when it is effectively blocking: marked `blocking`, not `minor`, and not prefixed `maybe:` or `unverified:`. It carries its severity, `class` and resolved `locations`, and every other finding lands in the suggestions.
...
- Any agent lane whose answer is missing or invalid after a clean exit makes exactly one repair dispatch. A repaired answer is used and noted, and an unrepaired one takes that lane's existing no-trustworthy-verdict path.
- A dispatch reads only its own answer file, from its own per-dispatch path. A valid-looking verdict object printed on stdout, an earlier round's answer and a concurrent dispatch's answer never become its verdict.
- Every runner in the capability table declares an answer channel. The seam resolves the runner and model once per dispatch and uses that one result for both the prompt and the spawn options, so a role's configured model is kept. A codex-mapped lane role runs with a `read-only` sandbox and receives `--output-last-message`.
...
- Codex does not enforce the answer's shape: `--output-schema` stays unused. A codex-mapped role whose final message is not the JSON object gives an invalid answer, which takes the repair round like any other.
- `answerFile` is a new member of the runner capability table. Its doc twin, `docs/noldor/agent-runtimes.md`, and that file's `templates/` twin change with it.
...
- A reviewer finding blocks only when the reviewer marks it `blocking` and it is neither `minor` nor prefixed `maybe:` or `unverified:`. Everything else, including Important items the reviewer would still merge over, sits in `suggestions`.
...
- Each lane's latest raw answer sits at `.noldor/cr/answers/<slug>-<kind>-<lane>.json`, for anyone debugging a round. While a dispatch runs, its answer is written under a per-dispatch name next to it.
...
1. *How does a lane hand back its answer: dug out of stdout by a tolerant reader, through the claude CLI's native structured output (`--print --json-schema`), or written to an answer file?*
   -> An answer file. It separates the answer from the chatter, so no heuristic has to find it, and it works on every runner that can write a file. Claude's native structured output ties every consumer to a recent Claude Code CLI, where an older one rejects the flag and every dispatch fails, and to its output envelope. Codex fits the same file with no widened sandbox. Its CLI writes the child's final message to the answer path itself (`--output-last-message`), so its read-only sandbox never needs write access. (D1)
...
5. *Where does the placeholder-finding filter live?*
   -> In the shared answer reader, before schema validation. Each lane names its text-bearing field (the reviewer's `findings[].message`, the verifier's `mismatches[]`, the ui-reviewer's `findings[].message`), so no lane can reintroduce the phantom. Filtering first lets a padded `pass` validate. A `fail` left with no real mismatch goes to repair instead of standing on a placeholder. (D5)
6. *Keep the answer file after it is read, or delete it?*
   -> Keep the latest one per lane. Each dispatch writes under its own id, and after reading, the lane renames the file to `<slug>-<kind>-<lane>.json`. The directory stays bounded, no dispatch deletes another's file, and it remains the only record of what the lane actually said once the sink has reshaped it. (D6)
