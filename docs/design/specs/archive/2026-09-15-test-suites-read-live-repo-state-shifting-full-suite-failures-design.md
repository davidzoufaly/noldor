...
9. `src/release/__tests__/preflight.test.ts` file duration drops materially from its 31s baseline, measured by `vitest run --reporter=basic`: **~5s run in isolation, ~14s inside the full parallel suite**. The earlier target of "under 10s in the suite" assumed `gh` and `npm` were the dominant cost; measurement falsified that. The residue is `git`, which this feature does not seam — see *What remains unresolved*.
