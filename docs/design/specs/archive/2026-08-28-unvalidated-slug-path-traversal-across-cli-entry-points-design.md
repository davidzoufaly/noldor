...
Reads go through `readFileNoFollow` / `readFileNoFollowAsync`, which open with `O_NOFOLLOW` so the refusal happens inside the syscall, where there is no gap to race — **for the final component**. An intermediate directory replaced with a symlink after validation is not covered, because `O_NOFOLLOW` constrains only the last component and closing the whole chain needs per-component `openat`, which Node does not expose. That residue is stated rather than implied: rewriting a directory inside the repository already requires write access to it, a strictly stronger position than the argument-supplied traversal this feature exists to stop. A platform that does not expose the flag is detected rather than silently degraded: a bitwise OR swallows an undefined flag (`1 | undefined === 1`), which would produce an ordinary symlink-following open while the code still read as guarded, so its absence throws instead.
...
2. `slugPath` returns `escapes-root` for a path resolving outside the anchor — including when `relRoot` itself is a symlink pointing outside — `unsafe-symlink` when the composed final segment is a symbolic link, whether its target exists or dangles, and `uninspectable` when it cannot `lstat` the segment for any reason other than `ENOENT`. A final segment that simply does not exist yet succeeds.
2b. A symlink planted at a vetted path *after* `slugPath` returned still cannot be read or written through: the no-follow IO pair refuses at the open.
...
8. Every CLI accepting a `--slug`/`--slugs` flag exits non-zero on an invalid value before any slug-derived protected operation; a missing flag value remains a usage error with its own exit code. Consumers that take a slug without building a path from it are out of scope and named as such: `archive-resolve.ts` matches it as an equality key, `discover.ts` filters a set with it, and neither joins it.
