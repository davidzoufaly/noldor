...
1. _What happens to the matching baseline sections once the rules exist?_ -> Keep the baseline untouched: it stays the always-on full-read surface, and churning a distributed, twin-synced page buys nothing. Rule-file discoverability flows through `docs/noldor/rules.md` (the templated cascade page each rule links), not through per-rule links back to the baseline (D1, rationale revised in CR round 5 after round 4 moved the links).
