...
A config found by the scan is skipped whenever its path matches a `TSCONFIG_CANDIDATES` name at the repo root — **unconditionally, not only when the root loop managed to grade it**. The distinction is load-bearing: a root candidate that fails to parse is *reported* (`tsconfig-invalid`) but never graded, and it is also discovered by the walk, so a skip keyed on "already graded" would emit a second `tsconfig-invalid` for the same file.
...
A weaker `warn` was considered and dropped. It rested on a blast radius that no longer exists: the fixture trees and deliberately-malformed configs it worried about are excluded by `FIXTURE_DIRS`, before any read is attempted.
...
13. `docs/noldor/rules.md` no longer claims the floor reads root configs only, and `templates/docs/noldor/rules.md` carries the identical correction — `pnpm noldor doctor` reports no template drift.
14. `pnpm noldor checks invariants` on this repo stays green, and no new runtime dependency is added to `package.json`.
...
8. _Does this need a `docs/noldor/rules.md` update?_ -> **Yes — a factual correction, mirrored into `templates/docs/noldor/rules.md`.** (D8) Not because of a severity exception (D3 removed that), but because the doc makes a claim the change falsifies: it currently states that "the floor reads root configs directly instead of resolving the `extends` graph". After this change the floor still resolves no `extends` graph, but it no longer reads only root configs. A `templates/` twin holds the identical sentence, so both must move together or `doctor` drift reds.
