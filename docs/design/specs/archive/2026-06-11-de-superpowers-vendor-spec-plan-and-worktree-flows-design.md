...
- Path rename `docs/design/` → `docs/design/{specs,plans}` — split back to the roadmap at promotion (`Path Rename: docs/design to docs/design` entry, recovered 2026-06-11). All `docs/design/` path strings stay valid.
- Historical artifacts: existing specs/plans under `docs/design/`, FD body prose, archived refactor-workspace fixtures keep their `superpowers` mentions.
...
- Spec stage (gate-invoked or standalone): invoke the `noldor-spec` skill; it dialogues to a design, then writes `docs/design/specs/YYYY-MM-DD-<slug>-design.md` per `pnpm noldor prep format spec`.
- Plan stage: invoke the `noldor-plan` skill; it writes `docs/design/plans/YYYY-MM-DD-<slug>.md` per `pnpm noldor prep format plan`.
