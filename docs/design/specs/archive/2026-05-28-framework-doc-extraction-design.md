...
- `docs/milestones/`, `docs/vision.md`, `ideas.md`, and `docs/design/{plans,specs}/` mix tracks similarly.
...
**Goal:** Framework plans/specs live in `packages/noldor/docs/design/`. Framework ideas in `packages/noldor/docs/ideas.md`. Framework vision in `packages/noldor/docs/vision.md`.
...
**Done when:** `find docs/design/specs -name '*.md' | xargs grep -l 'track: framework' | wc -l` = 0. `find packages/noldor/docs -name '*.md' | xargs grep -l 'track: product' | wc -l` = 0.
