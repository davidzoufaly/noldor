...
- **Rows are labelled.** Each app area gets one row, and a top-level text node above it is the row's label. The label is the pages' sibling, never their parent, for the same reason. Every top-level text node counts as a row label. The consumer picks the areas (charuy: Scene, Model pane, Bar & menus, Chat, Floor plan).
- **A row is a band.** It runs from its label's top edge down to the next label's top edge. Every page sits in exactly one band: its top edge inside the band, at least one label font size below the label's top (so the title never covers the page), and its bottom edge above the next label.
- **Titles are huge.** Every row label has `fontSize` of at least 200. On charuy's canvas that renders at 28–42 px at zoom-to-fit.
- **Twins sit together.** Inside a row, the pages of one state are neighbours, in the order the surface's `modes` lists them (charuy: dark, then light).
...
- `duplicate-id` — an id carried by more than one node. Names each such id.
...
- `twin-order` — checked only when the surface declares `uiCoverage.<surface>.modes`. A state whose pages are not neighbours, or not in `modes` order. Names the state.
...
- **Geometry that is not a number:** a top-level frame whose `x` or `y` is not a plain number cannot be placed in a row, so it is reported as `page-outside-row` with that reason. A frame whose `height` is not a number (sized to content) skips only the next-label rule — the check never reds what it cannot measure.
- **A label with no `fontSize`:** the editor's default applies, which is far under 200, so it reports `small-row-label`.
...
5. A top-level frame above the first row label, starting less than one label font size below its label, or reaching past the next label, fails both, naming the page.
6. A row label with `fontSize` under 200 fails both, naming the label and its size.
7. With `uiCoverage.<surface>.modes` declared, a state whose pages are split apart or out of `modes` order fails both, naming the state. Without declared modes, twin order is not checked.
