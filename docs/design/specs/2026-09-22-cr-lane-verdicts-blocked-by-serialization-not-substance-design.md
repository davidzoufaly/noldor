...
- Agent lanes hand back a structured answer through a channel that never mixes it with chatter. That channel is a JSON answer file, or, for a runner that enforces a schema itself (codex), that schema's validated final message. This replaces the two grammars in use today, fenced JSON and markdown buckets, both dug out of stdout.
...
A runner whose capability is `structuredOutput: 'schema'` (codex, `src/core/agent-runner/capabilities.ts:13`) writes no file. Its sandbox stays `read-only` (`src/core/agent-runner/runners/codex.ts:15`), and it answers through the registry's existing `schemaPath` path (`--output-schema`), which codex enforces itself. The lane validates that final message with the same zod schema. The JSON Schema file for each lane ships beside the lane, mirroring `src/cr/cr-record.schema.json`, and is pinned to its zod source by a parity test like `src/cr/__tests__/schema-parity.test.ts`. So no runner's sandbox is widened to let it write the answer.
...
As defence in depth, the shared answer reader drops placeholder entries before schema validation. A placeholder is an entry whose text, trimmed of brackets and punctuation, is empty or one of `none`, `n/a`, `no issues` or `nothing`, compared case-insensitively. Each lane names the field it filters: the reviewer's `findings[].message`, the verifier's `mismatches[]` and the ui-reviewer's `findings[].message`. Render-export reports no findings. Because the filter runs first, a `pass` padded with `["(none)"]` validates as the clean pass it is. A `fail` whose only mismatches are placeholders fails its refinement, so it is treated as an ambiguous answer and goes to the repair round (Unit 4) to name the mismatch.
...
- A dispatch reads only its own answer, from its own per-dispatch path, or from the enforced schema message for a codex-mapped role. A valid-looking verdict object printed on stdout, an earlier round's answer and a concurrent dispatch's answer never become its verdict.
- A codex-mapped lane role runs with a `read-only` sandbox.
