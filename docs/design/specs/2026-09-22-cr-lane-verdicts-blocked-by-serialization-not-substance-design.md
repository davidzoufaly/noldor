...
Who writes the file depends on the runner, and the capability table says which. `RunnerCapabilities` (`src/core/agent-runner/types.ts:22`) gains `answerFile: 'agent-writes' | 'cli-writes'`. claude, opencode and stub are `agent-writes`: the child writes the file with its own tools, which all three may do today (`bypassPermissions`, `--auto`, and an in-repo node process). codex is `cli-writes`. Its sandbox stays `read-only` (`src/core/agent-runner/runners/codex.ts:15`), and the codex CLI itself writes the schema-validated final message to the path. It does this through `--output-schema <schema>` plus `--output-last-message <path>` (codex-cli 0.156.0 has both), which `buildCodexArgv` gains as a `lastMessagePath` option. Because the field is a required member of an exhaustive `Record<RunnerName, RunnerCapabilities>`, a runner added later cannot compile without declaring its channel. So there is no runner with no way to answer, and no runner's sandbox is widened to let it write.

The prompt and the spawn options both depend on that channel, so the seam resolves the runner exactly once per dispatch, before building either. It calls `resolveRunner(role, loadAgentsConfig(cwd))` (`src/core/agent-runner/registry.ts:41`) and pins the whole result, runner and model. That needs `opts.runner` plus a new `opts.model` that `spawnAgent` honors alongside the pin. Today a pin keeps only the runner (`registry.ts:117`) and would silently drop a role's configured model. With both pinned, `spawnAgent` cannot resolve differently, and a model set under `agents.roles` survives. It then builds the matching answer instruction. For `agent-writes` that is "write your answer JSON to this path". For `cli-writes` it is "your final message is the JSON object". It also passes `schemaPath` and `lastMessagePath` only for `cli-writes`, which the registry's capability check (`registry.ts:121`) already requires. The JSON Schema file for each lane ships beside the lane, mirroring `src/cr/cr-record.schema.json`, and a parity test like `src/cr/__tests__/schema-parity.test.ts` pins it to its zod source. Either way, every lane reads its answer from the answer file, and none parses stdout.
...
- Codex's schema mode can be stricter than zod. It may require every property to be present, so a lane schema used there may need `nullable` rather than `optional` fields. The parity test pins each shipped JSON Schema to its zod source, so the two cannot drift, but the first codex-mapped run is where a strictness mismatch would show.
