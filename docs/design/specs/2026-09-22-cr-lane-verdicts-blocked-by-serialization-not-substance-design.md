...
- Every runner in the capability table declares an answer channel. The seam resolves the runner once per dispatch and uses that one result for both the prompt and the spawn options. A codex-mapped lane role runs with a `read-only` sandbox and receives `--output-schema` and `--output-last-message`.
