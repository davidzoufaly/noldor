...
| `pnpm noldor garden sdd-report`                    | Walks SDD detectors (1-13 + 19 + architecture), writes `docs/sdd-report.md`. Informational — never blocks                  |
...
| 3   | Features without spec                 | `noldor-tier == full` and `links.spec` empty (formerly flagged all FDs; now tier-gated)                                                                             | Author or link the spec under `docs/design/specs/`                                                    |
...
| 7   | Spec files not referenced             | `docs/design/specs/*.md` not pointed at by any FD `links.spec`                                                                                                 | Link the spec or archive it                                                                                |
...
| 16  | Plan without FD                       | Plan file at `docs/design/plans/<date>-<slug>.md` whose slug has no matching `docs/features/<slug>.md`                                                         | Promote the plan's feature onto a FD or archive the plan                                                   |
...
`/noldor-garden` adds 4 doc-maintenance signals on top (not in `pnpm noldor garden sdd-report`): stale plans (move to `docs/design/plans/archive/` once matching feature is `done`), stale specs (same trigger, `docs/design/specs/archive/`), unused backlog entries (drop or merge), and a deterministic-seed rule-contradiction sweep with an LLM false-positive filter.

> **Beyond the numbered 20.** `detectAll` ([`src/garden/garden-detect.ts`](../../src/garden/garden-detect.ts)) also wires in unnumbered detectors with no row above: architecture, skill-code-drift, fd-link-rot, fd-command-rot, malformed-fd, code-links-drift, migration-coverage, milestone-shipped-incomplete, bootstrap-override-audit, and undeclared-doc-impact (plus the four gate-compliance detectors below). The script is the source of truth; this numbered table is the stable-numbered subset, not the full census.
>
> **architecture** — **Signal:** one of the four pages in `docs/architecture/` (`context.md`, `containers.md`, `modules.md`, `flows.md`) is missing, carries no mermaid fence, declares a diagram kind the registry disallows, still holds a `<!-- TODO:` placeholder, or cannot be read. A separate advisory fires when `modules.md` stops naming a module the code has. **Fix:** `pnpm noldor docs architecture --check` prints every one; edit the named page (`manual-edit`). **Routing:** page findings reach `sddGaps` through `collectGaps`, so they gate the garden receipt and therefore a release, and independently the release preflight carries its own blocking `architecture` row that calls the check directly — restamping the receipt does not get past it; module advisories ride garden's own `architectureAdvisories` key, which is deliberately outside `FINDING_CATEGORIES` and never blocks. A repo with no `docs/architecture/`, or one whose scaffold nobody has edited, reports nothing at all.
>
> **fd-command-rot** — **Signal:** a `phase: done` FD body quotes a `pnpm`/`noldor` invocation that no longer resolves against the live CLI surface (manifest ∪ `package.json` scripts ∪ script catalog). **Fix:** correct the invocation in the FD (`manual-edit`) — or, when the FD is deliberately quoting a *rejected* design option rather than a live command, mark that line with `noldor-fd-command-rot-ignore` (conventionally the HTML comment form, at end of line or alone on the preceding line). The marker is the twin of `skill-code-drift`'s `noldor-skill-drift-ignore`. Suppression is per *line*, so a marked markdown paragraph exempts every command it quotes, not just the rejected one — keep the rejected option in its own paragraph when the surrounding prose cites live commands worth checking. **Why it exists:** without it the only way to silence a road-not-taken citation is to de-backtick the prose, degrading the FD to satisfy a detector — so recording why an option lost cost a permanent SDD gap. A fence delimiter is never suppressed; put the marker on the command line inside the fence.
>
> **skill-code-drift** — **Signal:** a skill markdown file under `.claude/skills/` or `templates/.claude/skills/` cites a `pnpm <script>` no `package.json` defines, a `noldor <group> <sub>` the manifest lacks, or a repo-relative path that is gone. A fourth class, `non-portable-script`, fires on a **fenced** command block in a **shipped** skill naming a script only *this* repo's `package.json` defines — it resolves in every test here and breaks the first consumer that runs it, because the framework installs skills but not scripts (Q-0239, found releasing charuy v0.7.0). Shipped means the file is under `templates/.claude/skills/`, or under `.claude/skills/` with such a twin: a consumer receives this detector but no `templates/` tree, so the skills it wrote are never judged against a premise that is false for them. **Fix:** rewrite the block against `pnpm noldor …`, or mark it with `noldor-skill-drift-ignore` — on the line, or alone on a line above the opening fence to exempt the whole block, which is the form to prefer for a command block so the marker does not travel with anything a reader copies. **Routing:** the first three classes stay advisory (`action: 'investigate'`) in `garden detect`; `non-portable-script` is additionally enforced at commit time by `pnpm noldor checks skill-portability` (`validate.skill-portability` pre-commit job), the only blocking half. Inline backtick spans in prose are never checked for portability — naming `pnpm release` in a sentence is a reference, not an instruction.
>
> **malformed-fd** — **Signal:** a `docs/features/*.md` whose frontmatter does not parse against `FeatureFrontmatterSchema`. **Fix:** `pnpm noldor features validate` names the schema error; edit the FD (`manual-edit`). **Why it exists:** the stale-plan/stale-spec ownership chain treats an unparseable FD as an owner of *unknown phase* — it neither archives nor ages out the plans and specs that FD may own — so without this row one malformed FD would silently thin the staleness report.
>
> **undeclared-doc-impact** — **Signal:** a first-parent `fast-track` commit changed a file an FD's `links.code` owns and carries no `Noldor-Doc-Impact:` line, and that FD — `phase: done` with a written `## Usage` — has recorded no commit since: none `commitsForFeature` finds (its scope or a `Noldor-FD:` line), and none whose `Noldor-Doc-Impact:` names it. Only commits after the oldest one carrying a declaration count, so a repo that never used the declaration reports nothing. **Fix:** read the FD's Usage against the listed commits; update it, or when it still holds, commit `<!-- noldor:usage-checked <sha> -->` under its `## Usage` as `docs(features:<slug>): Usage checked against <sha>` through a micro-chore (`manual-edit`). **Routing:** its own `undeclaredDocImpact` key, outside `FINDING_CATEGORIES`, so it never blocks the receipt or a release; a git failure is reported as a finding rather than an empty result. The ship-time half is gate Step 4's doc-impact check, which runs `pnpm noldor features owners` and records the declaration.
