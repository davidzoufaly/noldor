...
>
>
> **fd-command-rot** — **Signal:** a `phase: done` FD body quotes a `pnpm`/`noldor` invocation that no longer resolves against the live CLI surface (manifest ∪ `package.json` scripts ∪ script catalog). **Fix:** correct the invocation in the FD (`manual-edit`) — or, when the FD is deliberately quoting a *rejected* design option rather than a live command, mark that line with `noldor-fd-command-rot-ignore` (conventionally the HTML comment form, at end of line or alone on the preceding line). The marker is the twin of `skill-code-drift`'s `noldor-skill-drift-ignore`. **Why it exists:** without it the only way to silence a road-not-taken citation is to de-backtick the prose, degrading the FD to satisfy a detector — so recording why an option lost cost a permanent SDD gap. A fence delimiter is never suppressed; put the marker on the command line inside the fence.
