...
- **Validation** — once `.noldor/id-counter.json` exists, `validate:triage` errors on a missing `id` (`missing-entry-id`), a malformed `id` (`malformed-entry-id`), and the same `id` carried twice (`duplicate-entry-id`): by two entries across both files, by an entry and a feature MD's `entry-id:`, by two feature MDs, or by a live entry and a key of `.noldor/retired-entry-ids.json` — a feature MD may keep a retired key only when the record's `slug` is its own. A branch rebase can merge a mint race without a conflict (both sides moved the counter by the same step), so this error is the only layer that sees it. With no counter file, missing `id` is silent — a consumer that hasn't opted in isn't blocked.
- **References** — `blocked-by:` bullets (legacy alias: `deps:`) may reference an ID (`Q-0042`) or a slug interchangeably; `resolveEntryRef` (`src/triage/entry-id.ts`) resolves an ID to its slug (scanning roadmap + backlog, then FD `entry-id:` frontmatter). An unknown ID resolves to itself and counts as unshipped, the same failure mode as a typo'd slug. `validate:triage` additionally flags refs that resolve to no known entry ID, entry slug, or feature MD (`unknown-blocked-by-ref` — advisory, error under `--strict`), and flags a roadmap entry whose blocker sits *below* it in file order (`blocker-ordered-after-dependent` — same severity; file order is priority, so that order schedules the dependent first). `/noldor-garden`'s `circular-blocked-by` detector flags cycles in the blocked-by graph. Independently of ordering, `next-priority` holds back any entry whose `blocked-by` still names a queued roadmap entry — it drops out of every bucket, is listed under `blocked` in the `--suggestions` JSON, and is named on stderr — unless every entry is blocked (a cycle), when nothing is held back so the queue never reads as empty.
...
For `/noldor-promote` (roadmap/backlog → feature MD) see [`workflow.md`](workflow.md). For `/noldor-garden`, `pnpm noldor garden sdd-report`, and the 15-detector contract see [`garden-and-drift.md`](garden-and-drift.md).
