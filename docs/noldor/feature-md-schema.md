...
| `opt-in`      | `string[]` (min 1)                  | `.noldor/config.json` keys that switch the feature on in a consumer; any one satisfied means enabled. `a.b` = present and non-empty, `a.b=v` = equals `v` or is an array containing it (e.g. `crLanes.code=render-compare`). `pnpm noldor doctor` warns when a roadmap/backlog entry's `- parent:` names an FD whose opt-in is unset in this repo and every `consumer.knownConsumers` repo — check the extension has a user before sizing it. Hand-added; absent = always on. |
