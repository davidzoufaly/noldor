...
- **warn** — the requirement is a real migration: `noUncheckedIndexedAccess` (221 errors when first probed against this repo), `exactOptionalPropertyTypes` (47). These are a ratchet — scheduled work that stays visible on every run — because hard-failing a commit on multi-day work only teaches people to bypass the hook. Also warn: `tsconfig-absent`, `manifests-unreadable` and `lib-inherited`, where the floor could not resolve its own input and the honest report is "unchecked" rather than a verdict. `lib-inherited` fires only when *no* root candidate declares a `lib` — one that doesn't is normal, since a base config exists to be extended and the root that extends it owns the type surface. What it catches is the standalone `target: ES2023` project with no `lib` anywhere, which used to pass in silence.
...
**A waiver is keyed by id alone, so it applies repo-wide — to every config the floor grades, not just the one that prompted it.** Since the `lib` half is asserted against every discovered `tsconfig*.json`, waiving `lib-es-builtins` for one legacy package also downgrades it for the root config and every sibling package. That is a real widening of what a single waiver buys, and the `warn` line is the only trace of it. Waivers stay id-keyed deliberately — a `{ id, path? }` shape is a config-schema change with its own validation and migration — so a repo that needs a narrower exception should raise the offending config's `lib` instead of waiving.
...
The scanner is hand-rolled rather than delegated for two independent reasons. Taking a JSONC package to read two config files would contradict `platform-over-dependency`, the rule this invariant exists to make enforceable. And TypeScript 7 exposes no in-process JS parser API — its root export carries `version` alone, with parsing behind the tsgo API server — so `ts.readConfigFile` is not callable here even if a dependency were acceptable. The same constraint is why the floor grades each config's **declared** `lib` instead of resolving the `extends` graph to an effective one. Discovery is not limited to the root, though: the `lib` half is asserted against every `tsconfig*.json` the workspace scan finds, because `lib` replaces rather than merges across `extends`, so a compliant base protects only those children that declare no `lib` of their own. Fixture directories (`__tests__`, `fixtures`, `test`, `tests`, `e2e`, `__fixtures__`) are skipped — their configs exist to be malformed. What stays approximate is the inherited case: a config extending a `node_modules` preset, an in-repo base not named `tsconfig*.json`, or a bare `target` default escapes, and `nestedLibViolations` documents that rather than claiming completeness.
