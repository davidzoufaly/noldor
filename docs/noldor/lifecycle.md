...
flowchart TD
    ...
    Feature -->|"specs-only-new"| Spec["📐 docs/design/specs/<br/>(noldor-spec)"]
    ...
    Spec -->|"full-*"| Plan["🛠 docs/design/plans/<br/>(noldor-plan)"]
...
| **Spec**     | `docs/design/specs/<date>-<slug>-design.md` (non-trivial only)                              | `noldor-spec` skill (skipped when complexity verdict = `skip-brainstorm`)                     |
| **Plan**     | `docs/design/plans/<date>-<slug>.md`                                                        | `noldor-plan` skill                                                                           |
...
| **Audit**    | `docs/sdd-report.md` (15 gap categories)                                                         | `pnpm noldor garden sdd-report` (informational; release script regenerates as precondition)               |
...
| `in-progress → done` (release auto-restore) | `pnpm release` runs `fillMarkers` and detects `phase: in-progress` + `introduced` already set + changelog block this cycle.  | Asymmetric counterpart to the attach revert. Squash-merge would otherwise erase the (in-progress) signal — see the changelog-pr-flow-integration spec §3.5. Sets `updated = <newVersion>`. |

See the changelog-pr-flow-integration spec §3 for the full asymmetric state-machine rationale.
