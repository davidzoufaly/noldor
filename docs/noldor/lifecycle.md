...
```mermaid
flowchart TD
    Idea["📝 ideas.md<br/>raw bullets"] -->|"/triage<br/>(reads docs/vision.md)"| Roadmap["🗺 docs/roadmap.md<br/>flat priority list<br/>(file order = priority)"]
    Idea -->|"/triage<br/>(parking lot)"| Backlog["📋 docs/backlog.md<br/>parking lot"]
    Backlog <-.->|"manual move"| Roadmap

    %% /gate is the universal entry for all paths
    Roadmap --> Gate["/gate<br/>(pick one of 6 paths)"]
    Backlog --> Gate
    ...
    ReviewGate -->|"commit<br/>Noldor-Path: &lt;path&gt;<br/>pre-commit: validate:features + sync"| Main["main branch<br/>FD: phase → done<br/>/draft-feature-md --refresh"]
...
The six gate paths map onto the diagram above. Every change picks exactly one path via [`/gate`](../../.claude/skills/gate/SKILL.md) before any implementation:
...
| **Garden**   | stale plans, unused backlog, rule contradictions, invariant violations                           | `/garden` skill, backed by `pnpm noldor garden detect`                                                    |
