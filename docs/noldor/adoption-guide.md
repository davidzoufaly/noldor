...
| ------------ | ----- | ------------- | --------- |
| Node.js | ≥ 24 | `bin/noldor.mjs` executes every CLI surface from `dist` | nothing runs |
...
| `categories`        | Release-notes categories (functional-domain axis, NOT commit types). Default `["Core","Tooling","Other"]`. Grows via `/noldor-triage` + `/noldor-promote`. |
...
| `knownConsumers`    | Roots of other Noldor repos (absolute or relative to this one) whose `.noldor/config.json` `doctor` reads to tell whether a feature's FD `opt-in:` keys are set anywhere. This repo is always included. Optional. |
...
| `uiBoot`            | Per-surface boot recipes for the `render-compare` CR lane: `{ "<surface>": { "verifyCommand": "<verifyCommands server entry>", "route": "/…", "page": "<FINAL: page name>", "screenshotCommand": "… {url} {out} {width} {height}", "maxDiffRatio": 0.25, "captureTimeoutMs": 60000 } }`. Keys must be declared in `uiSurfaces`; every placeholder substitutes single-quoted; routes carry a narrow charset. Optional. Pair with `autonomous.renderCompareMode` (`"advisory"` default \| `"blocking"`). Full contract in [cr-pipeline.md](cr-pipeline.md#render-compare-lane). |
