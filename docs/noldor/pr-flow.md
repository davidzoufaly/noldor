...
1. Try `gh pr merge --auto --squash` first (the happy path when auto-merge is enabled + checks gate the merge). GitHub removes the head branch on this leg only when the repo's delete-branch-on-merge setting is on and gh never touches it, so once the poll reports `MERGED` pr-flow probes `git ls-remote --heads origin refs/heads/<headRefName>` and deletes a surviving ref itself (`git push origin --delete`), reporting `deleted remote branch <b> (auto-merge left it behind)`.
2. On any non-zero exit, retry with `gh pr merge --squash` (synchronous — the merge happens immediately, no polling needed). `--delete-branch` rides along **only from the main checkout**: the flag makes gh check out the base branch locally before deleting the head branch, and from a linked worktree that checkout always fails with `fatal: 'main' is already used by worktree at <main-workspace>` — taking the remote-branch delete down with it. So pr-flow probes worktree context first (`git rev-parse --path-format=absolute --git-dir --git-common-dir`; the two paths differ only in a linked worktree) and, when inside one, withholds the flag and deletes the remote ref itself via `git push origin --delete <headRefName>` (best-effort — a failure only warns, since the repo's auto-delete-head-branches setting may have beaten it to it). The *local* branch stays for the gate's Step 4 cleanup, which runs `git worktree remove` + `git branch -D` + the `main` fast-forward from the main workspace, where checking out `main` is legal. From the main checkout the delete is gh's to do, but the flag's local post-merge step (check out the base, pull, delete the local head) can fail — a dirty tree is enough — and takes the remote delete down with it. So pr-flow probes afterwards — `git ls-remote --heads origin refs/heads/<headRefName>` — and deletes a surviving ref itself, reporting `deleted remote branch <b> (gh --delete-branch left it behind)`. It used to only warn, and the warning went unactioned: `origin/micro/*` grew one branch per micro-chore (Q-0219). The probe is best-effort: it stays quiet when `ls-remote` itself fails rather than deleting a ref it could not see. Every delete on every leg runs only after `gh pr view` has confirmed the merge, so a failed merge keeps its branch.
