...
Both take normalized geometry documents (`geometryDocSchema` in [`src/cr/geometry/geometry-doc.ts`](../../src/cr/geometry/geometry-doc.ts)) — no pen, no browser, no lane involved. Three families are compared. Each side's values collapse into representatives, then the two representative lists are matched by a single order-preserving forward scan, which on sorted lists is already maximum-cardinality optimal. The family tolerance (`edges` 2px, `fontSize` 1px, `spacing` 1px) is a budget spent ONCE across both stages — half to the cluster's permitted width, half to the match — so the guarantee is: two values differing by **more than the tolerance can never pair**, a difference at or under half the tolerance always pairs, and in between it depends on the neighbouring values. Clustering by width rather than by distance to the previous value is what stops linkage chaining a representative away from the values it speaks for. Closest-pair greedy is not: design `{0,3}` against impl `{2,5}` at tolerance 2 matches fully, but taking the smallest difference first pairs 3 with 2 and invents two unmatched values. What is left over is the verdict, against a per-family budget defaulting to 0:
...
Exit 0 within budget, 1 on drift, 2 when a document could not be read or parsed, is empty, or disagrees with the other on viewport. One caveat on reading a `fail`: the unmatched COUNT is exact, but where several values sit within tolerance of each other the value it names can be a neighbour of the true intruder — the report lists every value on both sides, so read it rather than the single label. The blind spot is deliberate and worth knowing: a node that relocates onto an alignment value the surface already uses moves no value into or out of either set, so moving a card between two columns of the same grid is invisible, while nudging it off its own column past the tolerance is caught. The lane that runs this per surface against a booted app is parked as roadmap entry `Q-0180`; the design of record for it is `docs/design/specs/archive/2026-08-25-ui-design-review-lane-geometry-compare-design.md`.
