...
4. Narrow *which* entries the run may ship — a different axis from `--max-features`, which bounds top-N in priority ORDER: `--size XS` (comma-separated, case-insensitive, validated against the triage size enum) and `--only <slug,slug>` (every slug must resolve against the queue **and not be parked** — a parked slug errors with its park reason and the `noldor autonomous unpark` remedy, since `nextItem` would never select it). Both apply to `--source roadmap` only and are refused elsewhere; a value matching nothing is an error rather than a run that ships nothing and exits 0. Narrowed-out entries appear in the skip log with the narrowing as their reason; the success oracle's universe stays unnarrowed.
