...
- `pnpm noldor checks skill-portability` — the blocking half, wired into `pre-commit` (`validate.skill-portability`). Exit 1 on any `non-portable-script` row: a **fenced** command block naming a script only this repo's `package.json` defines. Skills ship to consumers, which receive the CLI but none of these scripts, so such a block resolves in every test here and breaks the first consumer that runs it. The detector's other three classes stay advisory.
