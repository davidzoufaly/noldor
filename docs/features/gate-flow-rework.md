...
As an operator (human or agent) driving `/gate`:
...
- I want explicit confirmation before `/gate` commits to a heavy scaffolding path, so that I can cancel before `/promote`, worktree, and brainstorm fire.
...
The /gate flow was reworked with an overhaul of Step 0, Step 1.5, and Step 2.

#### PRs

- #6: /gate flow rework — Step 0/1.5/2 overhaul ([link](https://github.com/davidzoufaly/charuy/pull/6))
