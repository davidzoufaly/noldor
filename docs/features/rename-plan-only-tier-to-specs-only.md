...
As a framework operator or contributor, I want the medium-depth tier label to reflect what it actually produces (a spec, not a plan-without-spec), so that the AskUserQuestion prompt at `/noldor-gate` and `/noldor-new-feature` time, the path identifiers (`specs-only-new` / `specs-only-attach`), and the FD frontmatter agree on the same vocabulary.

## Usage

User-visible touch points after this lands:

- `/noldor-gate` and `/noldor-new-feature` ask `"FD creation depth — specs-only (no brainstorm) or full (spec + brainstorm)?"` instead of the prior `plan-only (no spec)` wording.
