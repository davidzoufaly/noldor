...
- Editor association: `noldor init` seeds `workbench.editorAssociations: {"*.pen": "pencil.designEditor"}` into the consumer's `.vscode/settings.json`, merging into whatever is already there and never overwriting an existing `*.pen` entry (that case is reported as a warning). A `.pen` is plain UTF-8 JSON, so without the association VS Code has nothing stopping it from rendering the design as text.
