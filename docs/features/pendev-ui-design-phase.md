...
- Spec drift (automatic at gate Step 2.5 on UI-bearing sessions): `pnpm noldor design verdict --pen <the .pen> --check` exits 0 while the spec is the one the design was approved against, 1 with the spec diff once it has changed, and 2 when the approval cannot be checked (no record, or one from before spec binding — take the verdict again with `--spec`). When the design still depicts the changed spec, `pnpm noldor design verdict --pen <the .pen> --reconfirm` rebinds it (the `.pen` itself must be unchanged); otherwise revise the design and take the verdict again.
