...
As an operator driving `/noldor-gate` (human or agent controlling the framework), I want the worktree branch to land on `main` via an auto-merged GitHub PR instead of a direct local merge, so that every integration boundary carries a PR audit trail, runs the CR pipeline as the merge gate, and can be cross-machine-enforced via GitHub branch protection.

## Usage

**`/noldor-gate` end-of-flow (post-implementation, all non-micro-chore paths)**

1. With session marker active and worktree branch ready, signal "ready to ship" to `/noldor-gate`.
...
**`/noldor-gate` micro-chore path (no worktree)**
...
- Direct push to `main` blocked locally → look at session marker; if PR flow should be running but isn't, ensure `/noldor-gate` end-of-flow was invoked; otherwise use the release override only for actual releases.
...
**`/noldor-gate full-attach` / `specs-only-attach` (enhancement on a done FD)**

1. Operator selects `full-attach` or `specs-only-attach` in `/noldor-gate` Step 1 and supplies the parent FD slug.
...
- #3: activate PR-flow bootstrap (lefthook + /noldor-gate skill + allowlist) ([link](https://github.com/davidzoufaly/charuy/pull/3))
