...
validator, or test will fail (e.g. `pnpm typecheck`, `pnpm test`),
run that exact command first and quote its actual output in the blocker
entry. If the command passes — or does not exist — do not emit the
