...
export const SPEC_FORMAT = [
  ...
  '- A UI-bearing spec (UI verdict `required`, no waiver) adds a `### Design coverage` H3 under ## Design: a `| Page | Criteria | Shows |` table (Shows is optional prose the check ignores) with one row per `FINAL:<surface>: <state>` page (in backticks) and the acceptance criteria it shows (`1, 4–6`, or `—`), plus at most one `not drawn` row for criteria with no screen of their own. Every criterion appears on some row; `design verdict --coverage` checks the table against the design, and `--approve` refuses a design it does not cover.',
  '- ## Acceptance criteria (testable bullets — a numbered list on a UI-bearing spec, whose coverage table cites them by number) / ## Risks / trade-offs',
