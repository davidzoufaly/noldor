 ...
 */
export const edgeScrollVelocity = (clientY, viewportHeight, threshold = 80, maxSpeed = 16) => {
...
export function shouldInsertBefore(rect, clientY, isFirstRow) {
  if (isFirstRow) return clientY < rect.top + rect.height;
  return clientY < rect.top + rect.height / 2;
}
function ctxFromTable(table) {
...
function combinedEtag() {
  const meta = document.querySelector('meta[name="combined-etag"]');
...
function wireDrag(ctx) {
  ...
  tbody.addEventListener('dragover', (ev) => {
    ...
    const overRow = ev.target.closest('tr[data-slug]');
    ...
    // firstElementChild.
    const firstNonDragging = tbody.querySelector('tr[data-slug]:not(.dragging)');
  ...
  tbody.addEventListener('drop', (ev) => {
    ...
    const allRows = Array.from(tbody.querySelectorAll('tr[data-slug]'));
...
async function sendMove(ctx, slug, targetIndex) {
  try {
...
function wireButtons() {
  document.addEventListener('click', (ev) => {
    ...
    if (action === 'remove') {
      ...
      const table = btn.closest('table[data-section]');
...
async function sendRemove(btn, section, slug, etag) {
  btn.disabled = true;
  try {
...
function wireDescriptionToggles() {
  document.addEventListener('click', (ev) => {
    const btn = ev.target.closest('button.description-toggle');
    if (!btn) return;
    const cell = btn.closest('td.description');
    if (!cell) return;
...
function wireEntryIdCopy() {
  document.addEventListener('click', (ev) => {
    const btn = ev.target.closest('button.entry-id-copy');
  ...
  document.querySelectorAll('button.entry-id-copy').forEach((btn) => {
    btn.hidden = false;
  });
}
...
function syncOverflow(cell) {
  const clamped = cell.querySelector('.description--clamped');
  if (!clamped) return;
...
function wireDescriptionOverflow() {
  ...
  const ro = new ResizeObserver((entries) => {
    for (const entry of entries) {
      syncOverflow(entry.target);
    }
  });
  cells.forEach((cell) => ro.observe(cell));
}
async function sendButton(btn, action, slug, etag) {
  const url =
    action === 'promote'
...
function init() {
  document.querySelectorAll('table[data-section]').forEach((table) => {
