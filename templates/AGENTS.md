...
- `design` — Running design context for a spec/plan dialogue (ledger + inline block): archive, ui-sync, capture, pen-bridge, arch-route, arch-progress, open, verdict, context, graph-context, log, support-check, geometry-diff, geometry-export, geometry-validate
