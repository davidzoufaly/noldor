...
| `tsconfig-absent` / `manifests-unreadable` | the floor's own inputs were found                   | warn      | a repo with no root tsconfig may be a deliberate pure-JS consumer; "no manifest readable" must not read as "react is not a dependency".    |
