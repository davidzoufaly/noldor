...
| `lib-es-builtins`               | a declared `lib` reaches es2025                                | **error** | the `platform-over-dependency` rule mandates `Object.groupBy`, `Promise.withResolvers`, Set operations, iterator helpers and `RegExp.escape` by name; every one is a TS2550 under `lib: ["ES2023"]`, so below this floor an enforced rule requires code the compiler rejects. |
