...
| `lib-malformed`                 | a declared `lib` is an array                                   | **error** | `"lib": "ESNext"` is a plausible typo that `tsc` rejects outright; reading it as "no lib, therefore inherited" skipped both lib checks on a config that does not compile. |
