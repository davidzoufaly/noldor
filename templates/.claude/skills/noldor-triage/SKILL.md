...
In-progress work (FDs with `phase: in-progress`) is tracked via FD frontmatter, not via a roadmap entry. `/noldor-triage` never writes to roadmap entries representing in-progress work — promotion of a roadmap entry to a feature MD happens via `/noldor-promote`, which removes the entry from the roadmap when scaffolding the FD.
...
4. **For each** untriaged bullet, first decide: **new entry** or **merge into existing**?
   ...
   - **Disposition follows the candidate `kind`:**
     ...
     - `feature` (disposition `parent`) → the idea overlaps an already-promoted FD; propose a **new entry** (roadmap/backlog per the rubric) carrying `- parent: <fd-slug>` so a later `/noldor-promote` attaches it (`*-attach`). Never sub-bullet-merge into an FD.
   ...
   - On a **new entry**, propose:
     ...
     - **target** — `roadmap` (with a position annotation: `top`, `after:<slug>`, or `bottom`), `backlog`, or `now`. `now` declares ship-next intent: the row is written as a roadmap insert at `top`, and after step 8's validations pass the skill auto-chains `/noldor-promote <slug> --tier=<full when size is L/XL, else specs-only>` — closing the old two-step seam (triage → manual `/noldor-promote`). Propose `now` only when the operator's bullet explicitly signals immediate work ("do this now", "next up"); never infer it from score alone.
...
- **Optional `parent: <existing-fd-slug>` field** in schema-C blocks signals attach intent at `/noldor-promote` time. `/noldor-triage` doesn't enforce parent existence; `/noldor-promote` validates and presents attach-to-parent option to operator.
- **Always** run the regen chain in step 8, even if zero ideas triaged (catches drift from manual edits).
- **Confidence + blocked-by are silently optional.** Missing `confidence` defaults to `med` at scoring time. A missing `blocked-by:`/`deps:` bullet means the dependency factor is `1.0` (no discount). `validate:triage` does NOT complain when either is missing — but a `blocked-by` ref that is *present* yet resolves to no known target is flagged (`unknown-blocked-by-ref`). Both fields can be backfilled by hand later — `/noldor-triage` will pick them up on the next run.
...
- **Score is derived, not stored.** The score column in the confirmation table is recomputed from bullet fields on every `/noldor-triage` run. Do not write `- score: <int>` into schema-C blocks. If the formula in `src/triage/score.ts` tunes, the new score takes effect on the next run without rewriting the markdown.
