...
4. **Render** the checklist. **Derive the sections from the payload, never from a list written here.** Emit one section per key whose value is non-empty, in the order `garden detect` emits them, plus the `Manual sweep` section from step 3 (the one section with no payload key). A key this skill has never heard of still gets a section — that is the whole point of the rule in step 1, and a hand-maintained roster is exactly how `architectureAdvisories` went unrendered for a release cycle.
   ...
   **Rendering `overrideAudit`.** It is one section, not one per override. `(M)` is `overrides.length` — every override listed, matching the row count under the heading. It is deliberately not `count`, which holds the UNEXPECTED subset only (`src/garden/detectors/override-audit.ts`: `count: unexpectedCount`); a repo whose `garden.overrideAudit.expected` rules absorb every override has `count: 0` with rows to show, and a `(0)` heading above three rows is a bug report waiting to happen. The unexpected/expected split belongs in the summary line below, not the heading. Follow the heading with a `•` summary line carrying `severity` and `<count> unexpected, <expectedCount> expected`, then one `•` row per entry in `overrides[]` showing `sha`, `reason`, and whether it was `expected`. Every row is advisory — the whole section is `•`, never `✓`.
