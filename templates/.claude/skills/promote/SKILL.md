...
4. Prompt the user for the user-facing release-notes **category** (one of `Modeling | Editor | Agents | Distribution | Docs | Tooling | Other`). The slug→category mapping is canonical in [`scripts/lib/area-category.ts`](../../../scripts/lib/area-category.ts) — this prompt suggests `areaToCategory(area)` as the default and the operator can override. Unknown areas fall back to `Other`. The dashboard `/backlog` view derives its Category column via the same helper, so keep the two in sync by editing the TS module (the SKILL.md no longer carries the table).
