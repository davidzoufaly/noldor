...
- **`--yes`** (optional, `--refresh` only) — non-interactive. Auto-apply drafts without prompting, per the decision rule in `--refresh` step 8. Used by `/noldor-gate` Step 4 in autonomous mode.
- **`--scope <comma-separated paths>`** (optional, `--refresh` only) — draft from exactly these files instead of reading `links.code` + `links.tests`. Used by `/noldor-gate` on attach paths to scope the parent-FD refresh to an enhancement's changed files.
- **`--usage-only`** (optional, `--refresh` only) — draft and write only `## Usage`; read `## User Story` for context but never modify or diff it. Used by `/noldor-gate` on attach paths.
...
- **Never modify Summary.** The Summary section is user-curated; `/noldor-promote` already copies the spec block's first paragraph.
...
| ---------------------------------------------------------- | ------------- | ------------------------------------------------------------- |
| Feature MD missing                                         | both          | Abort with: "Run `/noldor-promote` or `/noldor-new-feature` first."         |
...
- During `/noldor-promote` or `/noldor-new-feature` — those scaffold TODO stubs intentionally; the spec/code that would feed the drafter doesn't exist yet.
- On a feature with no spec AND no code yet (newly scaffolded via `/noldor-new-feature` with no body work done). The skill aborts in that case.
