...
1.5. **UI design step (predicate-gated).** Compute the UI verdict: candidate paths = the roadmap entry's `Touches:` values ∪ the FD's `links.code` (glob values expanded per `src/core/ui-predicate.ts` semantics), config = `consumer.uiPaths`/`uiSurfaces`, FD `design:` override absolute both ways. Write the verdict to the session marker (`uiVerdict`, `uiVerdictPaths`), and the pre-filter candidate set to `candidatePaths` — step 1.7 reads that, because `uiVerdictPaths` keeps only what matched `uiPaths`. On `skip`: add one line to the spec ("UI verdict: skip — <reason>") and continue to **step 1.7** — nothing else UI-related. (Never skip past 1.7: a repo with no `uiPaths` configured takes this branch on every session, so routing it to step 2 would make the structural read unreachable in exactly the repos that most need it.) On `required`:
   ...
   - **Editor unavailable:** only after the wake attempt above, stop for an explicit operator waiver — record it in the session marker (`uiWaiver: { reason, at }`) and in spec prose; never write the FD `design:` field. A session waived **at this point** produces no `.pen` and no `links.design` (a waiver taken later, at the verdict, is covered by the verdict bullet below). A closed editor and an absent editor look identical from Node, so waiving without the wake attempt buys permanent baseline debt for a fixable problem.
   - **Take the operator's verdict — the design is not settled until it is given.** Nothing outside this conversation distinguishes a design an operator ratified from one nobody saw, so the verdict is taken here and written down here. In order:
     ...
     A waiver taken **here** rather than at Seed — the bridge died after iteration — keeps the `.pen` that already exists and commits it with the spec, and **keeps the `links.design` that Record already wrote**. Record runs before the verdict and needs no MCP, so it completes even when the bridge dies; unsetting its pointer would strand the retained `.pen` outside the link-based archive flow, which is how `design archive` finds and repoints it. The link therefore names a committed-but-unapproved design, and the waiver note must say so in the spec, so a later reader does not mistake an archived `.pen` for a ratified one. The FD `design:` field is still never written — it is the operator's override, not a verdict record.
