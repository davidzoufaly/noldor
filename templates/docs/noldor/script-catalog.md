...
- **Outputs:** lane sinks + an aggregate verdict (exit 0 clean / exit 1 blockers / **exit 3 round-cap** — the budget is spent, so nothing was dispatched and the round history plus the `Noldor-Path-Override` remedy print instead). `orchestrate` also appends one round entry per resolved dispatch to `.noldor/cr/autofix/<slug>-<kind>.json`; it is the only appender. `escalate` drives retry / spawn-deep-review / override / abort. Driven by `/noldor-gate` Step 2.5 + Step 4.
