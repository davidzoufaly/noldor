...
- **Outputs:** `plan` prints `verdict:` (`auto-fix` / `decline`), `reason:`, `next:` (`reround` / `apply-then-stop` / `operator`), an authoritative `base-sha:` for the re-round, `round: n/2`, any `in-flight lanes:`, and the `[mechanical]` / `[design]` blocker split anchored at `file:line [lane]` with the reviewer's `suggestion:`; exit 0 = auto-fix then re-round, 11 = auto-fix the mechanical subset then stop (a design blocker rides along), 10 = decline, 2 = error. `record` appends a round and exits 0. The framework never applies a blocker — that is the gate controller's edit, made between the two calls.
