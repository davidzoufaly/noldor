...
The commit body is load-bearing in every row, because a subject line is what-only and `pr-summary-why-how-what` requires why and how too. `composeBody` composes deterministically and cannot author prose, so it surfaces what the commit already says. That body is no longer a matter of hope: `noldor-pre-push` rejects any outgoing code commit that lacks `Why —` / `How —` / `What —`, reading the stored objects rather than a provisional message — see [git-and-commits.md](git-and-commits.md) § Commit body contract. An empty body still degrades to a subject-only Summary rather than filler, which is now reachable only for grandfathered history, `--no-verify` pushes, and consumers that have not committed the activation snapshot.
