...
### Consumer-3 dogfood friction (2026-07-07, charuy re-adopt via GH Packages)

First adoption of the **published** `@davidzoufaly/noldor@0.5.0` (GH Packages, private) into a real de-noldor'd repo (charuy monorepo). Registry install + `init` + `doctor` + drive-3-changes (adoption PR #63, micro-chore #64, autonomous-drain feature #65, artifacts #66) all shipped. Each item below verified against noldor main before listing; overlaps with consumer-2 flagged.

- **[c3 #11, HIGH — drain] Drain re-picks an already fast-track-shipped slug → wasteful re-spawn loop.** `autonomous run --source roadmap` picked `pnpm-verify-playwright-browser-precheck`; the implementer shipped it via a **fast-track** PR (#65, branch `fast/<slug>`). Drain reconciliation only recognizes `feat/<slug>` FD-backed branches, so it never detected the merge → logged `salvaged orphan-remote-branch` → re-spawned the implementer 3× (retries-exhausted), ~171k tokens / ~13 min each, all correctly refusing to open a duplicate. Root causes: (a) merge-detection keys on `feat/<slug>` and ignores a merged `fast/<slug>` PR with the same slug; (b) fast-track PRs don't clear the roadmap entry, so drain re-selects it. Fix: treat a merged PR whose headRef ends in the target slug (any prefix) as shipped→skip; and/or fast-track end-of-flow clears the matching roadmap entry. Watch-signal: `shipped:0` + non-null `currentSlug` + `retries.<slug> >= 1` while a same-slug PR is MERGED. Touches: `src/autonomous/` (drain reconciliation/selection).
- **[c3 #9, HIGH — CORRECTS consumer-2 #16 "fixed"] fmt empty-target still fires for `graphify-out/*.json` (and any oxfmt-ignored glob-match).** Reproduced on main 2026-07-07: staging only `graphify-out/manifest.json` → pre-commit `fmt` runs `oxfmt --check <file>` → `.oxfmtrc.json` ignores `graphify-out/**` → `Expected at least one target file` → exit 1, commit blocked. The lefthook `exclude: '(graphify-out/|…lock…)'` regex is NOT filtering those paths out of `{staged_files}` (else lefthook would skip the job). `.md`-only commits are fine (md not in the fmt glob) — so the blast radius is graph/lockfile-only commits, not "any doc commit". Robust fix (semantics-independent): fmt hook must exit 0 when the resolved oxfmt target set is empty (e.g. a `noldor checks fmt-staged` wrapper that swallows the no-target error), OR fix the exclude to actually drop those paths. Touches: `lefthook/noldor.yml` + `templates/lefthook/noldor.yml` (+ maybe a `src/checks` wrapper + test).
- **[c3 #4/#6/#7, MED — init/doctor] Fresh adopt can't reach a green `doctor`: the `consumer.frameworkVersion` anchor is never written.** `init.ts:~109` stamps the anchor ONLY on a fresh scaffold, NOT on `--update`. A de-noldor'd repo with residual `.claude/` twins forces `init --update` (see c3 #2), so the anchor stays unset → `doctor` warns `framework skew: anchored (unset) ≠ installed 0.5.0 — run 'noldor upgrade'` → `noldor upgrade` fails `no frameworkVersion anchor … run 'noldor init', or pass --from` (circular; init already ran) → `upgrade --from 0.5.0` says `already at 0.5.0 — nothing to do` but does NOT persist the anchor. Had to hand-add `consumer.frameworkVersion`. Fix: `init --update` should stamp the anchor when absent; and/or `upgrade --from X==installed` should persist it. Touches: `src/cli/commands/init.ts`, `src/cli/commands/upgrade.ts`.
- **[c3 #10, MED — drain] `init` scaffolds no `autonomous` config block → drain hard-aborts.** `autonomous run` → `drain requires an 'autonomous' block in .noldor/config.json`, even though every field in `autonomousConfigSchema` has a default. Fix: `init` scaffolds a default `autonomous` block, and/or drain treats an absent block as `{}`. Touches: `src/cli/commands/init.ts` (config scaffold) or drain config load.
- **[c3 #3, MED — doctor] `doctor` reds (exit 1) on lefthook-not-on-PATH though lefthook is installed in `node_modules/.bin` and hooks fire.** The git-hook shim resolves lefthook via `node_modules`/`pnpm lefthook`; doctor's prerequisite check only probes global PATH → false-positive hard-red that would abort any doctor-gated adoption smoke. Fix: `src/core/prerequisites.ts` lefthook probe should also accept `node_modules/.bin/lefthook` / `pnpm exec lefthook`.
- **[c3 #1, LOW — doc] Adoption-guide install command omits `-w`, fails on any pnpm workspace.** `pnpm add -D @davidzoufaly/noldor` → `ERR_PNPM_ADDING_TO_ROOT … make it explicit … with the -w flag`. Guide should give the `-Dw` variant for workspace roots. Touches: `docs/noldor/adoption-guide.md`.
- **[c3 #2, LOW — init] `init` aborts on the FIRST residual file, one at a time, no full conflict list.** On a partial-residual repo you can't see the blast radius before choosing `--update`. Fix: enumerate all conflicting paths up front. Touches: `src/cli/commands/init.ts`.
- **[c3 #8, LOW — msg] `noldor upgrade` "no frameworkVersion anchor in .noldor/config.json" implies a top-level key; actual field is `consumer.frameworkVersion` (nested).** Name the full path in the error. Touches: `src/cli/commands/upgrade.ts`.
- **[c3 #5 — DUP of consumer-2 #6] placeholder consumer config, no inline field guidance.** Already triaged → `lockstep-packages-scaffold-vs-doc`; broaden that entry to "annotate load-bearing vs optional fields in the scaffold."
