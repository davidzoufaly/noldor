...
- repeated `cr orchestrate --kind code` runs across amend rounds APPEND a `Noldor-Reviewed-Subagent` trailer instead of replacing the existing one, so a commit that went through N CR rounds carries N receipts — all but the last stale (each amend changes `HEAD^{tree}`); pre-push validates against the tree, so a stale receipt is noise at best, false pass at worst. Fix: receipt amend should replace any existing trailer of the same key. Manual cleanup meanwhile: `git log -1 --format=%B | grep -v '^Noldor-Reviewed-Subagent:' > msg && git commit --amend --file=msg`, then re-run orchestrate. (surfaced shipping Q-0073, PR #268)
