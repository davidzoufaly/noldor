...
- gate should ADDRESS CR blockers instead of asking the operator to — both blocker seams are prompt-only today: artifact-stage Step 2.5's continue-dialog offers `address-blockers` whose prose is literally "operator edits the artifact" (`.claude/skills/noldor-gate/SKILL.md:161`), and code-stage `cr escalate` prompts `retry-implementation / spawn-deep-review / override-with-trailer / abort` (`src/cr/escalate.ts:67`); neither has an auto-fix outcome, so even `proceed-autonomous` + `autonomous.onFailure` cannot express "fix it and re-round". Add an auto-fix outcome (controller applies the blockers itself, re-runs orchestrate with `--base-sha <priorArtifactSha>`, surfaces the fix diff at the NEXT dialog rather than a question before it) behind a knob like `autonomous.onBlockers: 'auto-fix' | 'prompt'`, default `prompt` — design-disagreement blockers still need operator arbitration, mechanical ones (missing section, lint-class finding, unstated acceptance criterion) do not. Open question: how the controller classifies a blocker as mechanical vs design-disagreement without asking. (surfaced operator friction 2026-08-06) [triaged 2026-08-06 → gate-auto-addresses-cr-blockers]
...
- pr-flow's main-checkout leg should warn on a lingering remote branch, symmetric with the worktree leg — the worktree path now deletes the remote ref itself and reports it, while the main-checkout path just trusts `gh pr merge --delete-branch` and says nothing, so a gh-side delete failure is silent there. (surfaced CR of `pr-flow-worktree-checkout-skip`, PR #258) [triaged 2026-08-06 → post-merge-cleanup-reporting-gaps]
- `prep promote --ship`'s worktree skip message should name what it left behind — it prints `local main sync skipped` but not that the local feature branch is still present, so the operator does not know a `git branch -D` is outstanding. (surfaced CR of `pr-flow-worktree-checkout-skip`, PR #258) [triaged 2026-08-06 → post-merge-cleanup-reporting-gaps]
- a drain child that returns prose while a CR lane is still in flight is indistinguishable from a finished iteration — children 1 and 2 of the 2026-08-06 XS drain committed their work, then ended their turn with "waiting on the reviewer lane": no push, no PR, exit 0. `settleShipVerdict` reads that as a failed build (entry still in `parseAll()`, no open PR) and re-spawns a full rebuild at ~13min/~170k tokens each. Options: have the child assert a PR exists before returning, or teach the supervisor to recognize a branch-with-commits-but-no-PR as resumable (finish Step 4) rather than retry-from-scratch. (surfaced 2026-08-06 XS drain) [triaged 2026-08-06 → drain-false-retry-on-in-flight-cr-lane]
