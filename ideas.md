...
- `src/garden/garden-detect.ts` resolves a design artifact's owner FD through three steps with two opposite malformed-input policies. `loadFeatureBySlug` rethrows anything that is not ENOENT, so one FD with unparseable frontmatter aborts the whole `garden detect` run; the two fallbacks on the same chain — `scanFdsForOwner` and `resolveByGraphAdjacency` in `src/garden/plan-resolution.ts` — swallow parse failures and report no owner. Same input class, opposite handling, one chain: whether a malformed FD is loud or invisible depends only on which resolution step happened to reach it, and the age-out fallback then silently flags artifacts a malformed owner FD actually owns. Pick one policy — catch the parse failure at every step and treat it as no-owner (plus a separate loud FD-validity detector, which `features validate` already partly is), or make the fallbacks surface malformed FDs the way the primary does. Either direction changes public behaviour (`garden-detect.test.ts` currently asserts the throw propagates), which is why Q-0116 rejected it as out of scope rather than fixing it under a behaviour-preserving refactor. (raised 2026-08-20 at Q-0116 code review, round 2)
