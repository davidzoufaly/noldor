...
2. Surface to the operator: `Local main diverged from origin/main while the sweep PR was merging. Run 'git pull --rebase origin main' manually to reconcile, then re-invoke /noldor-release-sweep from step 9 (release confirmation).` Include `git log HEAD..origin/main --oneline` output so the operator sees what merged in.
