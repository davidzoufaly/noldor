...
4. **Tasks.** Each task: a **Files:** block (Create:/Modify:/Test: exact paths), then checkbox steps. One step = one 2-5 minute action. TDD order: write the failing test → run to verify FAIL (exact command + expected output) → implement → run to verify PASS → commit. Prescribe the commit as `git commit -F <message-file>`, where the file holds a conventional-commit subject, a blank line, a body with `Why —` / `How —` / `What —` sections (em dash, not `Why:` — a colon reads as a git trailer and gets absorbed; each section needs 24+ non-whitespace characters), then ONE trailing paragraph carrying `Noldor-FD: <slug>` and any other trailer. Do **not** prescribe `-m "<subject>" -m "Noldor-FD: <slug>"`: the blocking pre-push `summary-body` validator refuses a commit with no body, and a second `-m` opens a new paragraph so `git interpret-trailers --parse` sees only the last one and strands `Noldor-FD`. An executor following the plan verbatim must produce a commit that pushes.
