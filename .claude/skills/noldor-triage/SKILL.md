...
- **Score is derived, not stored.** The score column in the confirmation table is recomputed from bullet fields on every `/noldor-triage` run. Do not write `- score: <int>` into schema-C blocks. If the formula in `scripts/triage/score.ts` tunes, the new score takes effect on the next run without rewriting the markdown.
