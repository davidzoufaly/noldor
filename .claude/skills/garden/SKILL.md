...
7.5. **Code-link backfill prompt.** After the regen chain, surface any `Code files not referenced by any feature` SDD gaps as a separate decision: ask the operator if they want to run `pnpm noldor features fill-links-code-gaps` interactively to resolve them now (it produces a proposal MD for review, then `--apply` writes the updates). If yes, hand off to the operator — `/garden` does NOT execute it. If no, the gaps stay logged as Manual TODOs.
