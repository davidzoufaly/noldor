...
`pnpm lint` reads the checked-in `.oxlintrc.json`. `noldor init` scaffolds it once and the repo owns it from then on. Five rules are switched **off** there — each flags a pattern that is deliberate here, not a defect:
...
`jsx-a11y` and `react` are on even in a repo that carries no JSX — it is what makes the a11y and React claims below executable the moment it does.
...
| `disposable-lib`                | a declared `lib` provides Explicit Resource Management (`ESNext.Disposable`, or an `ESNext` / `ESNext.Full` umbrella) | **error** | widens the type surface only — cannot break existing code (probed: 0 new errors) — and it is what makes `using` compile.                  |
| `lib-es-builtins`               | a declared `lib` reaches es2025, counting only a whole annual library (`es<year>` or `es<year>.full`) | **error** | the `platform-over-dependency` rule mandates `Object.groupBy`, `Promise.withResolvers`, Set operations, iterator helpers and `RegExp.escape` by name; every one is a TS2550 under `lib: ["ES2023"]`, so below this floor an enforced rule requires code the compiler rejects. A granular entry does not count — `es2025.regexp` carries `RegExp.escape` alone. |
| `lib-malformed`                 | a declared `lib` is an array of strings                         | **error** | `"lib": "ESNext"` is a plausible typo that `tsc` rejects outright; reading it as "no lib, therefore inherited" skipped both lib checks on a config that does not compile. A non-string entry blocks for the same reason — grading the remainder would pass a config that does not compile. |
| `tsconfig-invalid` / `oxlintrc-invalid` | a config that is present parses and validates          | **error** | comments and trailing commas are tolerated, so a failure here means the repo owns a config its own toolchain cannot read. An unchecked blocking floor must not read as a met one. |
| `oxlintrc-absent`               | a React repo has an `.oxlintrc.json` at all                    | **error** | no config is the same end state as the plugin being disabled: zero React rules run.                                                       |
| `tsconfig-absent` / `manifests-unreadable` / `lib-inherited` | the floor's own inputs were found and resolvable | warn      | a repo with no root tsconfig may be a deliberate pure-JS consumer; an unparsed manifest must not read as "react is not a dependency"; and and a repo where **no** root candidate declares a `lib` leaves it resolving through `extends` or `target`, which this floor does not walk. Each is reported as *unchecked* rather than passed. (One candidate declaring no `lib` is normal — a base config exists to be extended — so `lib-inherited` is decided repo-wide, not per file.) |
...
- **React 19 spellings, not their React 18 predecessors** — on React 19+, which is where these exist at all. `ref` is an ordinary prop (no `forwardRef`), a context is its own provider (`<Ctx value>`, not `<Ctx.Provider>`), a ref callback may return its own teardown, and a mutation with pending/error state is `useActionState` rather than three `useState` calls around a `try/catch/finally`. `useActionState` gives you the state machine and a pending flag to disable the control with; it does **not** prevent a double submit, so a mutation that must not run twice still needs server-side idempotency. Full text, the version floor, and the rest (`use()` over effect-fetching, `useEffectEvent`) in the `react-19-idioms` scoped rule.
